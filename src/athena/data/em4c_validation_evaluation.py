"""EM-4C: the mandatory VALIDATION comparison -- deterministic (EM-4A)
vs. logistic (EM-4B) vs. the unconditional base rate, on real, sealed
VALIDATION data. Owner/Chief Architect gate: this is the FIRST script
in this workstream permitted to read VALIDATION outcomes, and only
because EM-4B is frozen (per the exact EM-4A -> EM-4B -> EM-4C
sequence). Never reads CALIBRATION or FINAL_TEST.

Scoring needs no numerical library (em4c_scoring.score_logistic is a
frozen dot-product + sigmoid over an already-fitted artifact) -- this
script runs on plain Python, same as the rest of the EM-4C evaluation
scaffolding it reuses unmodified (em4c_ranking/metrics/aggregation/
report). Read-only against EM-2's real VALIDATION evidence, EM-1b's
real VALIDATION labels, EM-3's real TRAIN-discovered register, EM-4B's
real frozen artifacts, and EM-1r3's real intraday evidence (for MFE/
MAE/time-to-target only); write-only against
`artifacts/research/em4c/` (git-ignored).
"""
...
from athena.explosive_move.em4c_scoring import score_logistic
...
def _build_joined_validation_dataset(*, em2_dir: Path, em1b_labels_dir: Path):
    rows: list[dict] = []
    label_by_key_all: dict[tuple[str, int], dict[int, int]] = {ft: {} for ft in FAMILIES_THRESHOLDS}

    em1b_groups = _grouped_by_instrument(em1b_labels_dir / "VALIDATION_checkpoint.jsonl.gz")
    invariant_groups = _grouped_by_instrument(em2_dir / "VALIDATION_session_invariant.jsonl.gz")
    dynamic_groups = _grouped_by_instrument(em2_dir / "VALIDATION_checkpoint_dynamic.jsonl.gz")
    ...
    while em1b_cur is not None and inv_cur is not None and dyn_cur is not None:
        ...
        for dyn_row in dyn_cur[1]:
            ...
            observation: dict = {
                "instrument_id": instrument_id, "session_date": d, CHECKPOINT_FIELD: cp,
            }
...
def _score_combo(
    *, family: str, threshold: int, rows: list[dict], eligible: dict[int, int],
    deterministic_rules: dict, bin_edges: dict, logistic_artifact: dict,
) -> dict:
    """Returns per-row deterministic and logistic scores for the
    eligible population, plus the raw ScoredObservation lists needed
    for every downstream metric."""

    det_scored: list[ScoredObservation] = []
    log_scored: list[ScoredObservation] = []
    cross_sections: dict[tuple[str, str], list[int]] = defaultdict(list)  # (session_date, checkpoint) -> row idx
    ...
    for row_idx, label in eligible.items():
        ...
        log_scored.append(
            ScoredObservation(instrument_id=row["instrument_id"], score=log_prob, label=bool(label))
        )
        cross_sections[(row["session_date"], cp)].append(len(det_scored) - 1)

    return {"det_scored": det_scored, "log_scored": log_scored, "cross_sections": dict(cross_sections)}


def _cross_section_k_metrics(scored: list[ScoredObservation], cross_sections: dict) -> dict:
    """Precision@K/Lift@K per real session-date x checkpoint cross-section
    (never pooled), summarized as mean/median across cross-sections."""
...
def _pooled_metrics(scored: list[ScoredObservation], *, is_probability: bool) -> dict:
    ...
    if is_probability:
        ...
        result["calibration_bins"] = [
            {"bin_index": b.bin_index, "predicted_mean": b.predicted_mean, "observed_rate": b.observed_rate, "n": b.n}
            for b in bins
        ]
    else:
        result["brier"] = None
...
def _compute_flagship_excursion(
    *, family: str, threshold: int, rows: list[dict], eligible: dict[int, int],
    ...
    """Real MFE/MAE/time-to-target for the flagship model's real
    POSITIVE-labelled VALIDATION observations, using real EM-1r3
    intraday candles -- forward_excursion.py unchanged."""
    ...
    # frozen em1b reference-price rule (event_labels.py / em1b_label_dataset_generation.py):
    # OPEN_TO_HIGH uses session_open[T]; TOUCH/CLOSE use close[T-1]. Both are
    # recoverable from this row's own checkpoint-dynamic evidence, never refetched.
    results = []
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="EM-4C VALIDATION comparison (deterministic vs logistic vs base rate)."
    )
    ...
    parser.add_argument("--out-dir", type=Path, default=Path("artifacts/research/em4c"))
    parser.add_argument("--skip-excursion", action="store_true", help="Skip the flagship MFE/MAE/time-to-target pass.")
    parser.add_argument(
        "--only", type=str, default=None,
        help="Comma-separated FAMILY:THRESHOLD pairs to score (e.g. TOUCH:10) -- for a fast canary run.",
    )
    ...
    print("loading EM-3 register + EM-4B artifacts")
    ...
    print("joining EM-2 VALIDATION evidence with EM-1b VALIDATION labels")
    rows, label_by_key_all, rows_processed = _build_joined_validation_dataset(
    ...
    if not args.skip_excursion:
        ...
        print("computing real MFE/MAE/time-to-target for the flagship (TOUCH_10)")
    ...
    summary = {
        "contract_version": "em4c-evaluation-v1", "partition": "VALIDATION",
        "source_run_ids": {
