...
| MP-NX6 | My Portfolio Owner Notes — persist owner-authored thesis / watch condition / reminder / review comment / follow-up on current holdings in `portfolio_holding_notes` (schema 19); visually and contractually separate from ATHENA evidence; notes never change Status, scores, or guidance; empty notes are deleted; notes follow NSE→BSE remaps (dest note wins on conflict) and are removed with the holding, confirm REMOVED, and My Portfolio reset. Review Session Mode is a later NX6 slice and is not started | 🔄 Owner Notes implemented 2026-09-08 — ready for Owner/Chief Architect review; Review Session Mode not started |
