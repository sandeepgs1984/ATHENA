...
- Sticky workbench correction pass: dashboard asset version advanced to `9.179.0`; the Current Holdings sticky header now uses an opaque high-z-index control deck with a top curtain so scrolled rows cannot bleed behind it, and the Sort reset is icon-only with a tooltip so it no longer reads as a third View mode.
- Initial-layout correction pass: dashboard asset version advanced to `9.180.0`; the sticky header curtain now starts hidden and is activated by a browser-side `sticky-engaged` state only after the Current Holdings header actually sticks, so KPI/header content is not covered on initial page load.
...
- Sticky workbench correction pass: dashboard asset version advanced to `9.179.0`; dashboard contract tests assert the opaque sticky header curtain, higher stacking layer, icon-only sort reset, and differentiated Sort/View controls.
- Final focused validation for the `9.179.0` sticky workbench correction: `node --check src/athena/api/static/js/08b-my-portfolio.js` passed; dashboard hosting / release-gate tests passed (`13 passed`); ruff passed on touched dashboard contract tests.
- Initial-layout correction pass: dashboard asset version advanced to `9.180.0`; dashboard contract tests assert the sticky curtain starts hidden, the `sticky-engaged` class is toggled from scroll/resize state, and the cache-busted assets advance together.
