...
|---|---|---|---|---|
| **0 — Foundations** | repo, domain, config, logging, calendar | `domain/` complete per §4; config loaders + invariants + feature flags + strategy profiles; JSONL logging; observability skeleton + system-health pre-flight; Calendar Engine with NSE holiday/expiry data | `athena today` prints correct CalendarContext for 10 test dates; config validation fails loudly; health checks pass | M |
| **1 — Data Foundation** | provider abstraction + SQLite storage | `MarketDataProvider` contract suite; FileProvider; validation (freshness, OHLC, gaps); corporate-actions; SQLite store + WAL + FKs + backup/restore | golden dataset ingests clean; corrupted data quarantined; database backup/restore verify clean | L |
| **2 — Market Context** | regime + universe engines | Regime Engine (volatility/trend/gap evidence); Universe Engine (investable universe construction + inclusion traces) | golden sessions classify correctly; universe generated has 100% explainable inclusion traces | M |
| **3 — Decision Intelligence** | indicators → decisions engine | Indicators (SMA/EMA/RSI/ATR); Evidence; Scoring; Confidence; Risk evaluation; Capital manager + sizing; 6 quality gates | decisions compose with trace; trade plans attach to trade decisions; all 6 gates validate | L |
| **4 — Orchestration & Ops** | scanner, scheduler, backtesting | Orchestration DAG pipeline; Daily Market Scanner; Watchlist manager; Strategy framework; Backtesting engine; Analytics | daily scan reports generated; scheduled runs execute; backtest runs reproduce outcomes deterministically | L |
| **5 — Portfolio & Execution** | capital management & abstraction | Portfolio engine (reserved/risk capital); Capital Allocation policy; Position Sizing; Order planning; Broker abstraction | portfolio metrics update; sector concentration caps enforce; broker-neutral orders batched | L |
| **6 — Derived Workspace** | intelligence workspace & reports | Operational reporting; derived console snapshots; explainability rationale; Timeline audit engine; Operational monitoring | reports browse; timelines causally ordered; stage telemetries observe; workspace snapshot compiles | M |
| **7 — Production Runtime** | coordinators & schedules integration | PipelineContract validation; PipelineCoordinator context threading; WorkspaceAssembler; SystemPipelineRunner; schedule runs | runner executes; stage failures isolate; scheduler history records and runs succeed | M |
| **8 — Application Platform** | REST APIs & RBAC security | FastAPI app factory; versioning `/api/v1/`; response envelopes; RBAC permission dependencies; User/API Key repos | auth tokens refresh; security endpoints block unauthorised requests; all REST routes validate | L |
| **9 — Workstation Console** | dashboard workstation UI | Vite/static files serving fallback; summary endpoints; portfolio/capital visualizations; market & calendar tab; decision traces | dashboard loads; charts update dynamically; universe inclusion trace modal inspects; console runs green | L |
| **10 — Live Dry Run & AI** | live scheduled paper trading | Real-time Quote/Candle ingestion validation; scheduled premarket/intraday cycles; alerts dispatch; AI weight tuning proposal | system runs daily on scheduler; live feeds ingest; daily trace briefings notify; AI playbook proposals generate | L |
