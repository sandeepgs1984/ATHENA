...
| ID-6E | Entry Qualification Replay & Shadow Validation — validate the frozen ID-6A–ID-6D.1 capability across deterministic historical replay (real engine) and persisted shadow observations; validation only, no methodology/threshold/API/UI change | 🔄 **ANALYSIS COMPLETE — READY FOR OWNER VALIDATION REVIEW 2026-09-02** — new read-only harness `src/athena/data/id6e_replay_shadow_validation.py` reuses ID-6B.1's own `ReadOnlyStore`/`candidates_at` unmodified but calls the real, closed `EntryQualificationEngine.evaluate()`/`resolve_evidence_finality()` (never re-derives the v0 formula). Replayed the identical ID-6B.1B window (10 sessions, 17,082 observations) for direct comparability: state distribution 76.17% NOT_YET / 21.70% QUALIFIED / 2.13% UNKNOWN; every headline statistic (QUALIFIED prevalence, WATCH 19.93%/TRADE 24.17% split, 39.76% flicker, checkpoint pattern) matches ID-6B.1B's research-formula figures almost exactly — confirms the closed engine faithfully reproduces the validated methodology, with no tuning performed to force agreement. All invariants hold (0 DISQUALIFIED_FOR_SESSION, 0 CONFIRMED_BY_POLICY, 100% NOT_EVALUATED confirmation, 0 non-v0 methodology versions), 0 harness defects, 100% LIVE_M5_PROVISIONAL finality (as expected for this WATCH/TRADE REGULAR-phase sample). Option C reconfirmed at full scale (EXPECTED_BAR_MISSING-flagged observations show near-identical state proportions to the unflagged population); M15 reconfirmed non-blocking (0.01% impact). Deterministic: two independent full runs produced a byte-identical analysis digest. Real `db/athena.db` shadow audit (read-only, `mode=ro`+`query_only=ON`): `entry_qualifications` table does not exist in production yet (schema not migrated) — reported honestly as `SHADOW_OBSERVATIONS_NOT_YET_AVAILABLE`, not worked around. Classification: **REPLAY_SOUND_SHADOW_EVIDENCE_INSUFFICIENT**. No profitability/outcome claim made. Report: `docs/research/ID-6E-ENTRY-QUALIFICATION-REPLAY-SHADOW-VALIDATION.md`. 18 new focused tests (2 mutation-verified), full repository suite 3,149 passed |
