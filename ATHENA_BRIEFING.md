...
| Explosive Move research (isolated, ADR-012) | `explosive_move/` — EM-1a immutable event/readiness contracts, EM-1r2 read-only corporate-action coverage/cohort evidence, EM-1r3 provider-free exact-slot intraday reconstruction/manifests, and EM-1r4 survivor-cohort admission/quote-timestamp hygiene (implemented, awaiting owner review — check `docs/MILESTONES.md` before relying on this as approved); no dataset labels, features, model, scanner, UI, canonical score input, or production recommendation |
