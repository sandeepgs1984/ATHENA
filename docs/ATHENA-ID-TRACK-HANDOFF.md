...
clock) implemented, tested (3,256 passed), and wired into the real
scheduled path, but **not yet active in the live production process**
(requires a restart, not performed) — a provider rate-limit audit found a
real pacing floor accounting for ≈63% of the observed average latency,
strong prior evidence for `INGESTION_DOMINANT`, pending actual measured
natural-cycle evidence. Status: **ID-7P0 INSTRUMENTATION READY — EVIDENCE
ACCUMULATION PENDING**. Full detail:
