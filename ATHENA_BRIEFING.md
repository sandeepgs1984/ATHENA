...
| Data | `data/` (providers incl. institutional flow file/NSE adapters, official NSE corporate-action adapter and bounded ingestion, validation, corporate actions, strict versioned index-constituent loader, SQLite `store/repository`) |
...
| Explosive Move research (isolated, ADR-012) | `explosive_move/` — EM-1a immutable event/readiness contracts plus EM-1r2 read-only corporate-action coverage, cohort, manifest, and replay materialization; no dataset labels, features, model, scanner, UI, canonical score input, or production recommendation |
