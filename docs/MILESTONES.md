...
| ID-6C | Entry Qualification Persistence — durable, auditable, append-only persistence for the EntryQualification observations the closed ID-6B.2/2A engine already concludes; no methodology change, no workflow wiring, no API/UI | ✅ **IMPLEMENTED — READY FOR OWNER PERSISTENCE REVIEW 2026-09-02** — new `entry_qualifications` table (SCHEMA_VERSION 16→17, `src/athena/data/store/schema.py`), FK-bound to `decisions(decision_id)`, mirroring `decision_traces`/`trade_outcomes` conventions exactly (enum `.value` strings, ISO-8601 tz-aware TEXT timestamps, order-preserving JSON arrays for `reason_codes`/`evidence_refs`). Composite primary key `(instrument_id, session_date, as_of, decision_id, methodology_version)` is the append-only logical/idempotency identity — `run_id`/`cycle_id` deliberately excluded (informational provenance only). `SqliteRepository.save_entry_qualification` is idempotent (no-op on an identical repeat) and fails loudly (`RepositoryError`) on a genuinely conflicting payload at the same identity, inside one atomic write-locked transaction (no check-then-insert race). Read API: `get_entry_qualification` (exact identity), `latest_entry_qualification_for_decision`, `latest_entry_qualification_for_instrument_session`, `list_entry_qualifications_for_instrument_session` (full flicker-history audit view) — deterministic `as_of`-based ordering throughout, never insertion-time. No point-in-time (`as_of<=`) query added (deferred to ID-6E, not yet justified). Decision supersession remains explicitly unresolved, deferred to ID-6D. Design note: `docs/design/ID-6C-ENTRY-QUALIFICATION-PERSISTENCE.md`. 32 new focused tests (all non-vacuous — 2 mutation-verified), covering every state/finality/confirmation round-trip, ordered reason-code/evidence-ref fidelity, idempotency, conflict detection, append-only flicker history, migration, and FK enforcement. No production DB writes, no provider/workflow dependency, no methodology change |
