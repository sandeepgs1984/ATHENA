...
| **DX-4** `/darvax/` surface | DarvaX's own page + assets and its own authenticated API (`/signals`, `/signals/{id}`, `/scan`), plus a bounded scan service. Auth **delegated** to ATHENA (no second credential store). Experimental label on every payload and in the page banner. Touches no ATHENA dashboard asset. | 🔄 Ready for review |
