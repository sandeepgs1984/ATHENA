...
- The dashboard Export popover contains Dataset and Format selectors, a
  private-values notice, status feedback, and one Download Export action.
...
- `rtk pytest tests/api/v1/test_my_portfolio_import_api.py tests/api/platform/test_dashboard_hosting.py tests/api/platform/test_decision_chart_release_gate.py`: PASS, 85 tests.
...
Implementation complete and focused validation passed; ready for Owner / Chief
Architect review.
