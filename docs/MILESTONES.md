...
| **DX-6a** Screening engine | Eligibility taxonomy (ACTIONABLE/WATCH/EXIT_RELEVANT/NOT_ELIGIBLE, pure functions of `signal_type` → DAR-CARD rules), **two** measured ranking quantities (distance-to-trigger, box height — bars-in-box and volume expansion deferred, see below), `darvax_sweeps` + `darvax_screen_results` + schema v3, 32 tests. **No sweep job, no UI, no API.** Design: `docs/design/DARVAX-SCREENER-DESIGN.md` | 🔄 Ready for review |
