...
| ID-6B.0 | Entry Qualification Methodology / Engine Design — discovery, evidence analysis, methodology design, and deterministic-engine contract only; decide what the future pure ID-6B engine should do before implementation | 🔄 **DISCOVERY COMPLETE — READY FOR OWNER METHODOLOGY REVIEW 2026-09-02** — methodology report created at `docs/research/ID-6B-ENTRY-QUALIFICATION-METHODOLOGY-DESIGN.md`. Recommendation: GO WITH CONDITIONS; implement no engine until the owner approves the v0 policy choices. No production code, persistence, workflow stage, UI, thresholds, provider calls, DB writes, EMR, DarvaX, ID-6C, ID-6D, ID-6E, or ID-7 work performed |
