...
| Intraday Intelligence (ID-track, foundation/evidence only — no EntryQualification, no Decision gate) | `session/` (ID-1: `SessionContext`, completed-candle rule, `session_day_start`/`session_open_close_ts`/`canonical_slot_candles` — the shared completed + canonical-slot authorities every consumer below reuses) and `intraday/` — `IntradaySignalSet` (ID-2, formalizes existing VWAP/5m-15m-confluence evidence), `OpeningRangeEvidence` (ID-3/ID-3.1: OR15/OR30, canonical-slot-integrity-corrected), `RelativeStrengthContext` (**ID-4, Ready for review, not yet Owner-approved — check `docs/MILESTONES.md`**: stock-vs-sector/market point-in-time comparative performance, not RSI). Wired into `OwnerValidationPipeline._scan_eligible`'s per-instrument `WorkflowStage` graph (`session`/`intraday_analytics`/`relative_strength`), never scoring/confidence/risk/decision |
