...
| ID-7E | ✅ ID-7E IMPLEMENTATION COMPLETE — READY FOR OWNER / CHIEF ARCHITECT REVIEW (2026-09-05). New `entry_actionability` `WorkflowStage` in `OwnerValidationPipeline._scan_eligible`'s per-instrument DAG, `depends_on=("entry_qualification",)` only (transitive-dependency guarantee proven structurally, not by insertion order). Reads the exact same-cycle Decision and the exact EntryQualification `entry_qualification_stage` produced this cycle -- never a repository "latest" query. `ind_stage` now also publishes `latest_completed_m5` (the exact completed M5 candle VWAP was computed from, via `session.latest_completed_candle` over the same bounded series -- never a second read); `session_vwap_as_of` is derived from that candle's own completion instant, naturally satisfying the engine's exact VWAP-provenance equality check. OR15 reuses `IntradaySignalSet.or15` directly. One captured wall-clock instant reused for both `evaluated_at`/`persisted_at`; engine invoked with `policy=None`. Persistence scoped to WATCH/TRADE (mirrors EQ's own persistence gate -- required, since EntryActionability's binding validation needs the referenced EQ to itself be a persisted row), preserving ADR-015's frozen WATCH->NOT_ACTIONABLE contract. 11 new tests: DAG/order proof, transitive-dependency structural proof, exact binding-identity proof, WATCH->NOT_ACTIONABLE, a full real-pipeline run reaching genuine ACTIONABLE with exact entry/VWAP/invalidation/reward/provenance values, UNKNOWN/INSUFFICIENT_EVIDENCE, UNKNOWN/INVALIDATION_UNAVAILABLE, a genuine contract error proven isolated to one instrument (Decision/EQ persistence from earlier stages survives; a healthy sibling instrument unaffected), idempotent re-run, and zero-currentness/zero-provider/zero-policy source-scan proofs. Full suite: 3599 passed, 1 pre-existing unrelated skip, 0 failures. `SCHEMA_VERSION` unchanged at 18; zero diff to schema/repository/`intraday/__init__.py`/the engine or domain modules; zero Decision/ID-6/EMR/DarvaX touches; zero new API/UI; zero currentness in the write path; zero provider calls; no production restart/replay performed. Not marked Owner-approved. **Owner/Chief Architect source review (2026-09-05): core workflow integration, same-cycle binding, M5/VWAP PIT composition, WATCH/TRADE persistence, and schema/repository reuse ACCEPTED; final closure HELD for one narrow production-scope defect -- ID-7E.1 authorized and completed same day, see row below.** ID-7F remains NOT STARTED, NOT AUTHORIZED |
| ID-7E.1 | ✅ ID-7E.1 COMPLETE -- ID-7E READY FOR FINAL OWNER / CHIEF ARCHITECT CLOSURE (2026-09-05). Defect: the engine was invoked for EVERY Decision type, with persistence merely gated to WATCH/TRADE afterward -- broader than the frozen production scope (TRADE/WATCH = the actionability funnel; every other Decision type = an architectural scope exclusion, never a methodology verdict). Fix: a new early scope gate (`if decision.decision_type not in (WATCH, TRADE): return {"entry_actionability": None}`) runs immediately after resolving the same-cycle Decision, strictly before any EQ/M5/VWAP/OR15 evidence is read or composed -- an out-of-scope Decision can never reach engine invocation or persistence, proven directly even when paired with a deliberately incoherent EQ. The later WATCH/TRADE persistence condition was simplified to unconditional (reaching that line already proves scope). EntryQualification/ID-6 unchanged. WATCH still evaluates+persists NOT_ACTIONABLE/UPSTREAM_DECISION_NOT_TRADE; TRADE still evaluates+persists across NOT_ACTIONABLE/UNKNOWN/ACTIONABLE regardless of EQ state (proven with a real, unforced non-QUALIFIED EQ). Stage name/depends_on/produces unchanged -- body-only correction, no DAG redesign. 5 new tests: out-of-scope engine/persistence non-invocation (call-count spies, not just an absent row), out-of-scope+poisoned-EQ completes cleanly, strengthened WATCH proof, TRADE+real-non-QUALIFIED persistence proof, DAG-unchanged structural proof. Full suite: 3604 passed, 1 pre-existing unrelated skip, 0 failures. `SCHEMA_VERSION` unchanged at 18; zero diff to schema/repository/engine/domain modules/ID-6/EMR/DarvaX; zero new provider/API/UI/currentness/config. `git diff --check`/`git status --short` clean; diff scoped to exactly `owner_validation.py` and its test file. Not marked Owner-approved. **ID-7E still pending final Owner closure; ID-7F remains NOT STARTED, NOT AUTHORIZED** **Owner/Chief Architect decision (2026-09-05): ID-7E.1 OWNER APPROVED / CLOSED, ID-7E OVERALL OWNER APPROVED / CLOSED -- canonical EntryActionability workflow integration frozen.** ID-7F0 discovery/contract-freeze authorized same day, see row below; ID-7F execution remains NOT STARTED, NOT AUTHORIZED |
| ID-7F0 | ✅ ID-7F0 DISCOVERY COMPLETE -- READY FOR OWNER / CHIEF ARCHITECT VALIDATION-CONTRACT DECISION (2026-09-05). Audited `id6e_replay_shadow_validation.py` in full: `run_replay` (fixed checkpoint grid, read-only `ReadOnlyStore`, exact-Decision-by-id loading, bounded reconstruction, the real unmodified engine, disposable JSONL/JSON output) directly reusable near-verbatim for ID-7F; `run_shadow_audit` (persisted-row integrity/distribution audit) is narrower than ID-7F0's own reconstruction-vs-production-comparison concept -- a genuine, honestly-reported finding, not assumed already proven. Froze the replay unit (exact EQ identity bound to its exact Decision at one checkpoint, never instrument/date alone), replay population (every persisted WATCH/TRADE EQ row, negative/UNKNOWN included), Decision/EQ binding (`get_decision`/`get_entry_qualification`, both already exist, zero new repository API), PIT reconstruction rules for M5/VWAP/OR15 (identical bounded reads/canonical helpers ID-7E itself already uses), `evaluated_at` replay semantics (one fixed injected value; excluded from shadow-equivalence fields, mirroring `repository.py`'s own existing exclusion), `persisted_at` verdict (`PERSISTED_AT_NOT_REQUIRED_FOR_ID7F`), the 20-field shadow-equivalence set, currentness exclusion, the determinism contract, and a 7-category failure-classification taxonomy. **Real-data finding (read-only query, zero writes/code committed): the live `db/athena.db` reports `schema_version=17` (ID-7A's schema-v18 migration never applied) and has no `entry_actionabilities` table; zero TRADE decisions have occurred since 2026-08-27 (before EQ persistence began), so zero real (TRADE, EntryQualification) pairs exist today -- WATCH is the only currently-available replay/shadow population; SHORT remains at zero real Decisions (100% of 96,985 TRADE decisions are LONG).** Classified reuse **B -- `SMALL_ID7F_ADAPTER_REQUIRED`** and defined its exact future contract (three functions) plus the frozen future test plan -- none implemented. Confirmed the exact deployment boundary (schema migration + `athena-serve` restart, neither performed), natural-shadow-only policy, staged sample-sufficiency policy (reusing ID-6E's own canary->one-full-session->review pattern, no invented minimum N), and deferred profitability/outcome validation to a separate future sub-milestone. Zero code/schema/repository/workflow changes; zero replay/shadow executed; zero deploy/restart/Validate-All; zero EMR/DarvaX touch. Full report: `docs/research/ID-7F0-ENTRY-ACTIONABILITY-REPLAY-SHADOW-VALIDATION-CONTRACT.md`. Not marked Owner-approved. **ID-7F execution remains NOT STARTED, NOT AUTHORIZED** **Owner/Chief Architect decision (2026-09-05): ID-7F0 OWNER APPROVED / CLOSED -- validation contract frozen, Classification B accepted.** ID-7F1 (Mode A historical replay only) authorized same day, see row below; Mode B shadow/schema-migration/deploy remain NOT AUTHORIZED |
| ID-7F1 | ✅ ID-7F1 COMPLETE -- READY FOR OWNER / CHIEF ARCHITECT HISTORICAL-REPLAY CLOSURE (2026-09-05). New module `src/athena/data/id7f1_entry_actionability_replay.py`: `run_replay(...)` reuses `ReadOnlyStore` (mode=ro/query_only=ON) and every canonical composition helper ID-7E's production stage already uses, calling the real unmodified `EntryActionabilityEngine` for every persisted WATCH/TRADE EQ row (exact identity, exact-by-id Decision binding, never "latest"). Every observation independently reconstructed+evaluated TWICE and compared for exact equality (genuine whole-pipeline determinism proof). Implements the frozen 4-category failure taxonomy plus honest `*_EMPIRICAL_VALIDATION_NOT_AVAILABLE` flags. Never calls save_entry_actionability; output is disposable JSONL/JSON under artifacts/research/id7f1/ (git-ignored). 33 new focused tests (pure-function + real-disposable-DB integration, including forced TRADE+QUALIFIED/non-QUALIFIED via the ID-7E test suite's own monkeypatch pattern, since the real DB has zero of either). Full suite: 3637 passed, 1 pre-existing unrelated skip, 0 failures. **Real replay executed against the real db/athena.db, strict read-only, complete population, 20.37s: 11,986/11,986 reconstructed, 0 defects, determinism holds on all 11,986, schema_version unchanged 17->17.** Population unchanged from ID-7F0 (100% WATCH, 0 TRADE) -- TRADE/ACTIONABLE/UNKNOWN/SHORT empirical validation confirmed still not available; additionally found 100% of real WATCH rows carry Direction.NONE. 100% of WATCH observations correctly NOT_ACTIONABLE/UPSTREAM_DECISION_NOT_TRADE (zero violations); UPSTREAM_EQ_NOT_QUALIFIED present on exactly the 10,073 non-QUALIFIED rows. M5/VWAP/OR15 available 93.55% of the time, descriptive only. Full report: `docs/research/ID-7F1-ENTRY-ACTIONABILITY-HISTORICAL-REPLAY-VALIDATION.md`. `git diff --check`/`git status --short` clean; diff scoped to exactly the new module and its test file -- zero schema/repository/workflow/Decision/ID-6/EntryActionability-methodology/EMR/DarvaX touches. Not marked Owner-approved. **Mode B shadow equivalence, schema v18 migration, and service deploy/restart all remain NOT AUTHORIZED** **Owner/Chief Architect source review (2026-09-05): the real 11,986-row historical replay result ACCEPTED IN FULL; final closure HELD only for a narrow harness-accounting/observability defect -- ID-7F1.1 authorized and completed same day, see row below.** Mode B/schema-migration/deploy remain NOT AUTHORIZED |
| ID-7F1.1 | ✅ ID-7F1.1 COMPLETE -- ID-7F1 READY FOR FINAL OWNER / CHIEF ARCHITECT CLOSURE (2026-09-05). Fixed an unsafe `rows_attempted = len(rows) + len(defects)` derivation (would double-count an observation that both reconstructs successfully AND carries a REPLAY_EQUIVALENCE_DEFECT; the accepted real run's own 11,986 figure was coincidentally correct since it had zero such mismatches) with explicit `population_total`/`duplicate_population_total`/`unique_population_total`/`rows_attempted` fields. Added a new `UnexpectedReplayException` diagnostic (deliberately separate from ID-7F0's own frozen defect taxonomy) for genuinely unexpected non-ValueError per-observation failures, catching only `Exception` (never BaseException/KeyboardInterrupt/SystemExit), continuing to the next observation, and forcing a new explicit `replay_acceptance` boolean False whenever any occur. The frozen ValueError->PIT_EVIDENCE_DEFECT rule and all PIT/VWAP/OR15/Decision/EQ reconstruction/evaluated_at semantics/engine call are unchanged. 7 new tests (determinism-defect no-double-count, pre-evaluation binding-failure accounting, duplicate accounting, unexpected-exception proof, ValueError-regression proof, infrastructure-failure-still-propagates proof, clean-population acceptance-true proof). Full suite: 3644 passed, 1 pre-existing unrelated skip, 0 failures. **Hardened harness re-run against the real db/athena.db, same strict read-only mode: 11,986/11,986 reconstructed, 0 defects of any kind, 0 unexpected exceptions, determinism holds, schema_version unchanged 17->17, replay_acceptance: true** -- population and distributions unchanged from the original run. Full report update: `docs/research/ID-7F1-ENTRY-ACTIONABILITY-HISTORICAL-REPLAY-VALIDATION.md`. `git diff --check`/`git status --short` clean; diff scoped to exactly the replay module and its test file -- zero schema/repository/workflow/Decision/ID-6/EntryActionability-methodology/EMR/DarvaX touches. Not marked Owner-approved. **Mode B, schema v18 migration, and service deploy/restart all remain NOT AUTHORIZED** **Owner/Chief Architect decision (2026-09-05): ID-7F1.1 OWNER APPROVED / CLOSED, ID-7F1 OVERALL OWNER APPROVED / CLOSED -- historical Mode-A replay frozen (11,986/11,986 accepted, replay_acceptance=true).** ID-7F2 (production activation + first canonical canary) authorized same day, see row below |
| ID-7F2 | 🔄 ID-7F2 PRE-ACTIVATION PREPARATION COMPLETE -- PRODUCTION ACTIVATION DEFERRED TO THE NEXT NSE TRADING-DAY, OWNER-OPERATED RESTART -- ID-7F2 REMAINS OPEN (2026-09-05). Pre-activation safety check confirmed schema_version=17, WAL mode, no RUNNING cycle, git HEAD at ID-7F1.1. Migration-source audit confirmed v17->v18 introduces only the already-approved entry_actionabilities table+indexes (SqliteRepository.initialize()'s existing idempotent mechanism -- no ad-hoc SQL). Pre-migration backup created+verified (db/backups/athena-pre-id7f2-schema-v18-activation-20260905T064233Z.db, integrity ok, 0 FK violations). 644 pre-activation tests passed. Live-process restart was attempted and correctly blocked by the auto-mode safety classifier -- reported to the owner rather than bypassed. Confirmed 2026-09-05 is a real NSE non-trading day (SessionType.WEEKEND) so no canary could occur today regardless. Owner decision: no live-process action today; restart deferred to an explicit owner-operated action at the next trading-day window. Exact stop/restart/verification procedure (A-G) documented: `docs/research/ID-7F2-ENTRY-ACTIONABILITY-PRODUCTION-ACTIVATION-CANARY.md`. Zero code/schema/methodology change; zero live-process action; zero EMR/DarvaX/ID-7F1 touch. Not marked Owner-approved. **Production activation, the first canonical canary, and Mode-B shadow equivalence all remain NOT YET OBSERVED / NOT AUTHORIZED** at that point. **Addendum (2026-09-07):** resuming ID-7F2 the next trading day found `schema_version=18` already active with 228 real persisted `entry_actionabilities` rows, with no owner-operated restart reported. A read-only forensic investigation (no restart/migration attempted) traced this via direct log evidence to the same `os.execv`-based restart mechanism ID-7P0.2 characterized — classification `EXECV_RESTART_CONFIRMED`, activation bounded between `2026-09-05T11:00:15+05:30` (ID-7E's commit) and `2026-09-06T10:23:07 UTC` (earliest real persisted row). The 2 non-today rows were fully traced to a pre-existing single-symbol "Validate" feature (`src/athena/ops/symbol_validate.py`) reusing the same `OwnerValidationPipeline`, not backfill (`HISTORICAL_BACKFILL_ABSENT`). Schema/integrity verified clean; zero out-of-scope/binding/duplicate/WATCH-invariant defects across all 228 rows; the first genuine post-activation canary (`run-premarket-20260907T081534`, 226 rows) matches expected population exactly. Acceptance: `ACTIVATION_PATH_PARTIALLY_UNPROVEN_BUT_RUNTIME_ACTIVATION_VERIFIED`. **Status: ID-7F2 ACTIVATION + CANARY VERIFIED — READY FOR OWNER / CHIEF ARCHITECT REVIEW; ID-7F2 REMAINS OPEN.** No restart/migration/repair performed. Full detail: `docs/research/ID-7F2-ENTRY-ACTIONABILITY-PRODUCTION-ACTIVATION-CANARY.md` §10. **Owner/Chief Architect decision (2026-09-07): ID-7F2 OWNER APPROVED / CLOSED.** **ID-7F3 (Mode-B production-vs-replay shadow equivalence) authorized and completed same day: ID-7F3 MODE-B PRODUCTION EQUIVALENCE EXACT — READY FOR OWNER / CHIEF ARCHITECT REVIEW.** `run_shadow_equivalence` added to `id7f1_entry_actionability_replay.py` (reuses Mode A's helpers) independently reconstructs every persisted production `entry_actionabilities` row and compares it against the persisted artifact on the frozen 19-field comparison mask (excludes only `run_id`/`cycle_id`/`evaluated_at`/`persisted_at`), classifying any mismatch (`PROVENANCE_MISMATCH`/`STATE_MISMATCH`/`REASON_MISMATCH`/`VALUE_OBJECT_MISMATCH`) rather than a generic failure. Full suite: 3736 passed. Run against real `db/athena.db` at `validation_cutoff=2026-09-07T04:15:24 UTC`: **488/488 reconstructed, 0 defects, 100% EXACT_MATCH, `mode_b_acceptance: true`, schema unchanged.** First-ever real `TRADE_QUALIFIED` (15) and `UNKNOWN` EA-state (9) evidence in the ID-7 track, all matching production exactly; `ACTIONABLE`/`LONG` still not observed. Zero writes/schema/methodology change; zero EMR/DarvaX touch. Full detail: `docs/research/ID-7F3-ENTRY-ACTIONABILITY-PRODUCTION-VS-REPLAY-MODE-B-SHADOW-EQUIVALENCE.md`. **Owner/Chief Architect decision (2026-09-07): ID-7F3 OWNER APPROVED / CLOSED** — `MODE_B_PRODUCTION_EQUIVALENCE_EXACT` accepted (488/488 EXACT_MATCH, 0 defects; ACTIONABLE/LONG still not naturally observed, non-blocking). **ID-7 OVERALL OWNER APPROVED / CLOSED same day** — the entire ADR-015/ID-7A0 through ID-7F3 track is closed. Same day, Owner/Chief Architect authorized **ID-8** (entry/risk methodology validation): **ID-8 DISCOVERY + EMPIRICAL METHODOLOGY CONTRACT READY FOR OWNER / CHIEF ARCHITECT REVIEW.** Real finding: all 60 currently-persisted real TRADE+QUALIFIED rows (2026-09-07, the first-ever real SHORT TRADE decisions) resolved to `UNKNOWN`/`INVALIDATION_UNAVAILABLE`, 100% — sharpening `LONG_VALIDATED_SHORT_UNVALIDATED` into a concrete EQ-methodology mechanism, reported not fixed (out of ID-8's boundary). Defined the full research contract (episode-first-checkpoint unit, theoretical entry proxy, leak-safe forward window, direction-aware MFE/MAE, a new `AMBIGUOUS_SAME_BAR` policy, 6-label outcome taxonomy, session-grouped statistical design) and ran a small 60-row read-only feasibility sample proving the mechanics work end-to-end (60/60 reconstructed, T1 35.0%/T2 13.3% hit rates — mechanical proof only, not methodology-validating, since this batch's risk geometry was invalid at entry). Zero source/schema/methodology/EMR/DarvaX changes. Full detail: `docs/research/ID-8-ENTRY-RISK-METHODOLOGY-VALIDATION-DISCOVERY.md`. **Owner/Chief Architect decision (2026-09-07): ID-8 Discovery contract OWNER APPROVED / CLOSED — FROZEN.** Full historical study authorized and completed same day: new committed `src/athena/data/id8_entry_risk_outcome_validation.py` independently reconstructed the real LONG population (fixing a real episode-construction bug found during self-validation), matching ID-7B.1's own published population-identity numbers exactly (6,624 episodes, 783 QUALIFIED) while several outcome rates (T1/T2 hit rates, median MFE) differ materially and are honestly classified `UNEXPLAINED_DIFFERENCE` (ID-7B.1's harness was never committed) — this new harness is now authoritative going forward. VWAP-loss occurred in 66.8% of valid-geometry observations; a strong session-time/T1-reachability association was found (`EXTENSION_EFFECT_WARRANTS_CALIBRATION`); native SHORT population (67 rows) remains 100% `UNKNOWN`/`INVALIDATION_UNAVAILABLE`, `LONG_VALIDATED_SHORT_UNVALIDATED` unchanged, zero ID-6 touch. Determinism proven (0 mismatches, independent rerun). 12 new tests, full suite 3756 passed. Classification **`ID8_V0_ENTRY_RISK_PARTIALLY_SUPPORTED`**. **Owner source review found LONG/SHORT pooling, a same-date-only (not canonical-session-close-bounded) forward window, a raw-SQL (not canonical) entry-candle lookup, and terminal ordering assignable without valid VWAP geometry — all corrected in place, same milestone, no ID-8.x created.** VWAP-loss semantics confirmed evolving/session-cumulative via direct citation (`ID-7B-ENTRY-RISK-METHODOLOGY.md:303-304`). MFE/MAE-before-event, full OR15 (22.64% event rate) and D1-ATR (1.72%, `EXACT_MATCH` to ID-7B.1's 1.76%) comparators, PIT regime association, a session-block bootstrap, and a chronological stability view all implemented. Reconciliation re-run LONG-only: population-identity and VWAP-loss/D1-ATR rates reconcile almost exactly; T1/T2/MFE/OR15-timing remain honest `UNEXPLAINED_DIFFERENCE`. Session-time finding reclassified `LATE_SESSION_TIME_BUDGET_EFFECT_OBSERVED`. Determinism re-proven; 19 tests (7 new); full suite 3763 passed. Classification unchanged, now on materially more rigorous grounds. **Second correction round (2026-09-07, same day, same milestone/files):** fixed a real MAE-strictly-before-target intrabar-ordering bug (a same-bar co-occurring adverse extreme could not be proven to precede the target touch, per OHLC's own information limits — the running tracker now snapshots strictly before folding in the hit bar's own value); investigated OR15/D1-ATR event semantics against every actual frozen source (methodology doc, `TradePlan._build_plan`, ID-7B.1's own uncommitted harness) and found neither recoverable — both withdrawn from event-rate/timing claims and downgraded to level/geometry-only comparators (`OR15_LEVEL_GEOMETRY_RECONSTRUCTED_EVENT_SEMANTICS_NOT_RECOVERABLE`/`D1_ATR_..._NOT_RECOVERABLE`), the OR15 case additionally finding the methodology doc's own text explicitly forbids treating it as an event ("never via breakout_event... semantics"); reconciled a flagged "19 vs. 20 sessions" bootstrap/chronological-stability mismatch, found to be a narrative writing error (not a code defect — both views always used the same 19-session population) with the bootstrap code hardened regardless. 24 tests total (6 added/1 removed); full suite 3768 passed. Rerun against real `db/athena.db` (schema 18→18, PID 2453 untouched): LONG population unaffected (783/756/0). New §32 directly answers the Owner's own freeze question — **PRIMARY LONG can be frozen independently of the unresolved legacy T1/T2/MFE discrepancy**; the harness is judged sufficiently self-consistent and source-grounded to be authoritative. **Third correction pass (2026-09-07, diagnostic-only):** fixed a sign defect in `t1_hit_bar_adverse_excursion_pct`/`t2_hit_bar_adverse_excursion_pct` (could go negative; adverse excursion must be a non-negative magnitude) by clamping at `max(0, adv)`. 2 tests added (26 total); full suite 3770 passed. Every other PRIMARY LONG field verified byte-for-byte unchanged by rerun (schema 18→18, PID 2453 untouched); all classifications/comparator verdicts preserved. Status **ID-8 PRIMARY LONG V0 ENTRY/RISK METHODOLOGY CORRECT AND READY FOR OWNER FREEZE**. Full corrected report: `docs/research/ID-8-FULL-HISTORICAL-ENTRY-RISK-OUTCOME-VALIDATION.md`. **Owner/Chief Architect decision (2026-09-07): ID-8 PRIMARY LONG V0 ENTRY/RISK METHODOLOGY OWNER FROZEN, ID-8 OVERALL CLOSED** — final classification `ID8_V0_ENTRY_RISK_PARTIALLY_SUPPORTED`. **ID-9 (position sizing/capital allocation) authorized and discovery completed same day: ID-9 POSITION SIZING DISCOVERY + V0 CONTRACT READY FOR OWNER / CHIEF ARCHITECT REVIEW.** Read-only discovery found `EntryActionability` already persists the exact entry/risk fields ID-9 needs (no artifact gap), a complete but entirely dormant-in-production P5.2-P5.6 sizing/allocation/order-planning pipeline already exists (operates on the pre-ID-track `Decision` object, fed by the legacy manual `owner_positions` ledger, never live capital), `CapitalConfig`/`RiskConfig` are loaded but unconsumed, no live broker cash/margin capability exists anywhere, NSE/BSE lot size is always 1 with no existing tick-size rounding convention, and recommends refusing SHORT sizing in V0. Proposes an unfrozen `PositionSizing` V0 contract and workflow-stage integration mirroring ID-7E's own precedent. Zero code/schema/production changes. Full report: `docs/research/ID-9-POSITION-SIZING-DISCOVERY-AND-V0-CONTRACT.md`. **Owner/Chief Architect decision (2026-09-07): ID-9 Discovery + V0 Contract OWNER APPROVED / CLOSED — V0 core implementation authorized same day.** **ID-9 V0 CORE IMPLEMENTATION COMPLETE — READY FOR OWNER / CHIEF ARCHITECT REVIEW**: new `src/athena/intraday/position_sizing_models.py`/`position_sizing_engine.py` (`PositionSizingV0Engine`, pure/deterministic, consumes `EntryActionability` state==ACTIONABLE directly); LONG-only (SHORT/NONE refused); three independent constraints (risk-budget/max-position-value/theoretical-deployable-capital), each floored to whole lots via `instrument.lot_size`, never forced to a minimum; `binding_constraints` reports every co-binding candidate; a genuine finding that theoretical-capital can only ever co-bind with max-position-value (never uniquely bind) since `max_position_value_pct <= 100%` always. `CapitalPolicy` is a small explicit caller-supplied contract (not `CapitalConfig`/`RiskConfig`, both confirmed dormant); `OwnerValidationPipeline` gained an optional `capital_policy` parameter defaulting to `None`, so production honestly reports `CAPITAL_POLICY_UNAVAILABLE` until the Owner supplies a real policy. New `position_sizing` `WorkflowStage` (`depends_on=("entry_actionability",)`) wired into the canonical DAG, no persistence (schema stays 18), no order/broker/execution activation. The dormant P5.2-P5.6 pipeline is classified `RETAIN_AS_LEGACY_COMPATIBILITY` in full and confirmed never imported by any ID-9 file — exactly one canonical sizing path exists. 42 new tests; full suite 3812 passed. Classification `ID9_V0_CORE_IMPLEMENTATION_COMPLETE_NO_PRODUCTION_ACTIVATION`. Full report: `docs/research/ID-9-POSITION-SIZING-V0-CORE-IMPLEMENTATION.md`. **Owner/Chief Architect source review (2026-09-07): V0 core sizing mathematics/architecture ACCEPTED in substance; closure held for three fixes, all resolved same milestone.** Fixed: (1) currentness enforcement — `PositionSizingV0Engine.evaluate` now requires a caller-supplied `CurrentnessResult` (the existing, unmodified `is_currently_usable`) and refuses `NOT_SIZED`/`UPSTREAM_NOT_CURRENT` for anything but `CURRENT`, since a persisted `ACTIONABLE` verdict is evaluation-time methodology only, never a live-currentness guarantee; one captured clock instant now serves both currentness `now` and `evaluated_at`; (2) `policy_version` now participates in `PositionSizing`'s own composite identity (`identity_tuple()`), with a refined presence rule distinguishing policy-never-inspected from policy-inspected-but-still-refused; (3) the silent `lot_size=1` fallback is removed — a contract error is raised instead when canonical instrument metadata is genuinely absent. 19 new tests; full suite 3831 passed. Zero schema change; zero new persistence. Classification `ID9_V0_CORE_IMPLEMENTATION_METHODOLOGY_CORRECT_NO_PRODUCTION_ACTIVATION`. Status **ID-9 POSITION SIZING V0 CORE IMPLEMENTATION CORRECTED — READY FOR OWNER / CHIEF ARCHITECT FREEZE DECISION**. Not marked closed; not activated in production. **Owner/Chief Architect decision (2026-09-07): ID-9 V0 core methodology/implementation FROZEN** (classification unchanged, nothing in the frozen list redesigned); ID-9 stays OPEN for one remaining operational gap only — production always supplies `capital_policy=None` — and **ID-9 V0 CAPITAL POLICY ACTIVATION** authorized same day, forbidding ID-9.1/ID-9.x/ID-10/reopening discovery/touching ID-6/ID-7/ID-8. **ID-9 V0 CAPITAL POLICY ACTIVATION (2026-09-07): READY FOR OWNER VALUES / FINAL CLOSURE.** New `src/athena/intraday/position_sizing_config.py` (`load_position_sizing_policy_config(config_dir) -> CapitalPolicy | None`, mirroring `athena.explosive_move.live.operational_config`'s own PATTERN) is the smallest canonical seam: `config/position_sizing_policy.json` (absent in production today), a 4-field shape 1:1 with `CapitalPolicy`, wired into the one canonical `_owner_validation_pipeline` helper both `_cmd_cycle` and `_cmd_serve` share — `OwnerValidationPipeline.__init__` unchanged. Absent → `None` → `CAPITAL_POLICY_UNAVAILABLE` unchanged; valid → one immutable `CapitalPolicy` reused for every instrument (proven by identity spy); invalid → `ConfigError`, never silent fallback; dormant `capital.json`/`risk.json` proven never read. Produced the required non-prescriptive Owner Policy Impact Table (₹10,00,000 normalized capital, entry=₹100, LOW/MEDIUM/WIDE risk × the specified 3×3 matrix, 27 rows, generated by the real engine) — reconfirming `THEORETICAL_CAPITAL` never uniquely binds. Policy-version discipline documented as an operational invariant, no registry built. 37 new tests (31 config-loader + 6 workflow-seam), 61 pre-existing ID-9 tests unweakened. Full suite **3868 passed, 1 pre-existing unrelated skip, 0 failures** (up from 3831). Zero schema change; zero new persistence; zero order/broker/execution/provider/network call; `db/athena.db`/PID 2453/EMR/DarvaX untouched; `git diff --check` clean. Production `capital_policy` remains intentionally absent, no value invented. Four exact values still required from the Owner: `total_deployable_capital`, `risk_budget_per_trade_pct`, `max_position_value_pct`, `policy_version` — activation is then a one-file, zero-code, zero-restart change. Status **ID-9 V0 CAPITAL POLICY ACTIVATION READY FOR OWNER VALUES / FINAL CLOSURE**. Not marked closed; does not start ID-10 |
