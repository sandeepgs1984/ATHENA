...
| **UX-2** Score/Confidence/Risk storytelling | Meaning over decimals: risk/score bands (Weak→Excellent), star-rated score contributors, confidence "why ATHENA trusts this" checklist, risk as categorized summary, safety gates as a reassuring checklist, a "Why?" contribution breakdown | None | 🔄 Ready for review |
| **UX-3a** Trade Plan visual redesign | Bigger, cleaner entry/stop/target/R:R presentation; new Expected Return % computed from the plan's own persisted entry/target values | None | 🔄 Ready for review |
| **UX-3b** Chart ATR/moving-average/volume overlay | Chart gains an ATR band, a moving-average line, and a volume bar subplot. Research (2026-07-26): volume already flows to the API today (tooltip-only in the UI); ATR/SMA exist internally but only as single point-in-time values on the D1 timeframe (TradePlan sizing) — plotting them on the 5m chart needs new `atr_series`/`sma_series` functions in `indicators/calculations.py` plus wiring into `market_history_service.py`'s existing 5m candle fetch. Small, additive, no ADR (API DTO extension, not a domain-object change) | Owner go-ahead already given (2026-07-26) | ⏳ Planned |
