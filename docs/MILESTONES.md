...
| **DX-6a** Screening engine | Eligibility taxonomy (ACTIONABLE/WATCH/EXIT_RELEVANT/NOT_ELIGIBLE, pure functions of `signal_type` → DAR-CARD rules), the four measured ranking quantities, `darvax_screen_results` + schema v3. **No sweep job, no UI, no API.** Design: `docs/design/DARVAX-SCREENER-DESIGN.md` | 🔄 In design — blocked on ADR-010 Amendment 2 |
| **DX-6b** Universe sweep + API | Single-flight owner-triggered background sweep (409 on concurrent start, cancellable, partial results preserved, per-instrument failure isolation), batched beneath `scan.max_instruments`; five `/screen` endpoints plus a `signal_type` filter on `GET /api/signals` | ⏳ Planned — blocked on DX-6a |
| **DX-6c** Screener UX | Tier groups, the box-range visualisation (floor/ceiling/close/trigger, inline SVG, no library), client-side sort/filter, and the honest states — no-sweep, running, cancelled-partial, skipped-with-reasons, stale as-of, methodology-digest mismatch | ⏳ Planned — blocked on DX-6b |
| **DX-6d** Universe-scale perf re-measure | Re-run the DX-4a harness against a 528-instrument sweep. **Not optional:** DX-4a measured 25 instruments and names universe-scale scanning as an explicit re-measure trigger | ⏳ Planned — blocked on DX-6b |
