...
| MP-RV3 | Copy-only pass: clearer investment/export descriptions, plain-language unavailable-filter labels and tooltips, and an explicit caption explaining overlapping triage categories; calculation and filter contracts unchanged | 🔄 Ready for Owner review 2026-09-12; static preview verified, full suite 4017 passed with one pre-existing macOS launcher failure; existing lint findings retained |
