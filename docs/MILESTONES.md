...
| **DX-3** Signal engine | Six-state box breakout/retest machine referencing the *topmost* box per Darvas rules A/B/C/D; three stop policies (canonical 10%, tight 1%, EMA ladder) with DarvaX's own EMA; `DarvaxSignal` persisted to `darvax.db` (schema v2) with computed explanation + evidence trace + methodology digest. **Never an ATHENA Decision.** | 🔄 Ready for review |
