...
| Portfolio & planning (no execution) | `portfolio/` — existing portfolio engine plus frozen My Portfolio V1 contracts (`my_portfolio_contracts.py`), PS-P2 generic import parsing/resolution (`imports.py`), PS-P4 sync orchestration (`sync.py`), PS-P5B/PS-P7B interpretation (`interpretation.py`), and PS-P7B confidence adapter (`confidence_adapter.py`) for CSV/XLSX current-holdings preview, canonical symbol resolution, reconciliation, server-owned snapshot math, background sync state, immutable analysis snapshots, freshness, provenance, holdings-digest currentness (`CURRENT` / `STALE_HOLDINGS_CHANGED` / `UNKNOWN`), Status, Key Trigger, Major Support / Exit, Target 1, Next Action, EntryQualification-gated ADD, inclusive stop-driven EXIT, PARTIAL/FAILED handling, stale snapshot preservation, compact explanation UX, and Conviction populated only from coherent persisted Decision Confidence HIGH/MEDIUM/LOW under `portfolio-interpretation-v1`. My Portfolio V1 is COMPLETE AND FROZEN as of 2026-09-03 after PS-P6C; PS-P7A is Owner/Chief Architect approved and frozen; PS-P7B implements Conviction only and leaves Trend / Setup, Support 1, Target 2/3, REDUCE, ROTATE, allocation, ranking, and history deferred. Isolated `portfolio_*` SQLite tables live in `data/store/schema.py`, API endpoints live under `/api/v1/my-portfolio`, dashboard UX lives in `api/static/js/08b-my-portfolio.js` and `api/static/css/05b-my-portfolio.css`, and `owner_positions` remains the legacy/manual-fill ledger. Also `allocation/`, `sizing/`, `orders/` (planning only), `brokers/` (abstraction only), `execution/` |
