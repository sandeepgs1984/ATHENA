...
| EM-7 | Run isolated shadow validation and OFF-vs-shadow performance comparison | 🔄 **EM-7 DISCOVERY COMPLETE — IMPLEMENTATION NOT STARTED 2026-09-03** — source-grounded discovery (no production code touched) found EM-7 has no owner-ratified contract beyond ADR-012 §10 + the roadmap doc's identical "live shadow validation" scope (compare EMR-disabled vs. EMR-shadow-mode canonical performance impact; track precision/lift/calibration/MFE/MAE/misses/false-positives/regime-drift/data-failures/latency). The actual gap after EM-6 is entirely operational: **zero live-invocation mechanism exists anywhere** (no CLI command, no scheduler, no cron entry for `run_scan_cycle`) — this, not modeling or presentation, is why `db/emr.db` remains absent. The scanner itself also has 3 real correctness gaps making it unsafe to run unattended today: no exception handling / no `FAILED`/`PARTIAL` run status (a crash leaves an orphaned `RUNNING` row forever), non-idempotent candidate/transition persistence (replaying a `run_id` duplicates rows), no concurrency lock. Data acquisition is already solved cleanly — `EmrMarketDataPort` already reads only already-ingested `db/athena.db` candles (mirroring DarvaX's ADR-010 read-port pattern), so an operational scanner would not duplicate provider traffic. Recommends **Option A**: a new, isolated, config-gated EMR scheduled worker (mirroring DarvaX's `enabled`-flag pattern), sequenced behind fixing the scanner-correctness gaps first. Recommends a new ADR/ADR-012 amendment and an EM-7A0→EM-7E sub-milestone sequence. 5 owner policy questions raised (checkpoint cadence, universe scope, hardening-as-own-milestone, config-gate design, worker-isolation pattern) — none source-answerable. Full detail: `docs/research/EM-7-DISCOVERY.md`. No ADR drafted, no EM-7A0 started, no `db/emr.db` created, no scan triggered, no scheduling added |
