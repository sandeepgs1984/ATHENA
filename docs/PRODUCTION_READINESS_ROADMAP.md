...
| Unattended ops | **R5 ready for review** — `athena run-due` + launchd/cron docs + failure alerts |

**Verdict:** Live Kite path approved. Unattended schedule + failure alerts implemented pending R5 owner approval.
...
| ID | Milestone | Objective | Exit criteria |
|---|---|---|---|
| **R5** | Host Schedule + Failure Alerts | launchd/cron (or equivalent) invokes `athena cycle` / `brief`; hard failures notify (webhook and/or email — advances DD-9) | ⏳ **Ready for review** — `athena run-due`, `docs/ops/HOST_SCHEDULE.md`, file+webhook alerts |
