...
| **DT-3** Tab restructuring (5 tabs) + spacing polish | Split "Decision History" into Response (Journal/Outcome) + History (Timeline + Analogs); Decision Timeline moved out of the always-visible hero (real wasted-space fix) | 🔄 Ready for review |
...
| Tests | ~25 new/updated dashboard-hosting assertions total across both rounds — a structural check slicing the Response/History tabpane bodies apart to lock in which panel (Journal vs. Timeline+Analogs) lives in which tab; a check that the now-empty `.decision-brief-hero`/`.decision-banner` no longer render from `renderDecisionBrief`; checks for the Recommendation tile, the ATHENA Summary card, and the View Details modal wiring. Full suite **1035 passed** |
...
| Status | 🔄 Built, tested, live-verified — awaiting owner review |
