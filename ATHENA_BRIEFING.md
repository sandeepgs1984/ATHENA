...
| DarvaX satellite (opt-in, ADR-010) | `darvax/` — self-contained: own `config`, `store` (own `db/darvax.db`, own schema version), `ports`/`adapters` (read-only view of ATHENA candles), `primitives`, `signals`, `scan`, `screening` (tiers, universe sweep, action classification, liquidity), `positions` (owner-recorded holdings; stop-loss frozen at entry, never deleted on close), `validation` (DX-5 outcome harness), `api` (own sub-app, own static assets — three views: Advisor / Levels / Table). ATHENA's only knowledge of it is `api/darvax_mount.py`, which reads one boolean |
