...
`docs/research/SI-P2E-ATHENA-DECISION-EXPERIENCE-DISCOVERY.md`,
`docs/research/SI-P2E-ATHENA-DECISION-EXPERIENCE-IMPLEMENTATION.md`,
`docs/research/SI-P2B1-STOCK-360-REFERENCE-REVAMP.md`,
`docs/research/SI-F0-FUNDAMENTALS-FOUNDATION-DISCOVERY.md`,
`docs/research/SI-F0.1-FUNDAMENTALS-SOURCE-FEASIBILITY-SPIKE.md`,
`docs/research/SI-F1-FUNDAMENTALS-EVIDENCE-PERSISTENCE-IMPLEMENTATION.md`,
`docs/research/SI-F2A-FINANCIAL-FACT-NORMALIZATION-DISCOVERY.md`,
`docs/research/SI-F2B-PRODUCTION-FACT-NORMALIZATION-IMPLEMENTATION.md`
...
| SI-P2D | Deterministic Written Research Summary — concise traceable narrative over frozen D1, structural, Decision, Portfolio, freshness, and capability evidence; no LLM, prediction, recommendation, or methodology | ✅ **COMPLETE AND FROZEN 2026-09-14** on `9.254.0` — Owner/Chief Architect final source and rendering review approved the fixed six-family composer/provenance contract, scannable Research Brief, responsive ATHENA/Data grouping, presentation-only NOT_HELD Portfolio-row omission with evidence retained, HELD context, and mode-aware GET/Analyze loading overlay; 16 focused/88 impacted integration tests, with the preceding full suite at 4094 passed |
| SI-P2E | ATHENA Decision Experience — evidence-first explanation of the latest persisted Decision with exact gate outcomes/details, optional TradePlan, independent Decision/plan/D1 freshness, no-Decision semantics, and Decision Brief/Evidence navigation; no Decision recomputation or contract expansion | ✅ **COMPLETE AND FROZEN 2026-09-15** on `9.277.0` — Owner / Chief Architect final visual and source review passed; Decision Evidence framing, persisted-direction coupling, and narrow SI identity/price/completed-D1 restacking are approved; SI-F0 and SI-N0 not started |
| SI-P2B.1 | Stock 360 reference-matched visual revamp and closure — presentation-only hierarchy, identity, chart glance, portfolio/capability, Price Map, ATHENA View, and DarvaX refinements over frozen SI-P2B evidence; no methodology/DTO/schema change | 🔄 **IMPLEMENTED / REVIEW-READY 2026-09-14** on `9.275.0` — identity hero and quote blend into the workspace; navigation is borderless, with readable destinations, compact docked symbol/price/date continuity, and a soft lower fade; the chart card owns one continuous canvas while plot and glance remain transparent children separated only by a faint divider; major cards and the bottom status rail avoid unnecessary outlines; persisted SuperTrend direction/value, aligned actions, polished terminal states, reference-proportioned rows, collision-safe Price Map, and ownership-safe terminal/helper copy remain; responsive/sticky/SuperTrend/collision geometry passes at 1579/1024/390px; mandatory ATHENA controls and frozen SI-P2C chart renderer remain preserved; awaiting Owner/Chief Architect review; not frozen |
| SI-F0 | Fundamentals Foundation Discovery — authoritative research across repository inventory, multi-attribute fact identity, bitemporal/versioned availability, source strategy (official exchange as canonical vs commercial as candidate normalization), regulatory freshness, sector scoping, and refined implementation roadmap | ✅ **COMPLETE AND FROZEN 2026-09-15** — architecture approved in `docs/research/SI-F0-FUNDAMENTALS-FOUNDATION-DISCOVERY.md`; architectural principles aligned with Owner review; initial scope approved (Non-Financial Mainboard First, Consolidated preferred with scope isolation, SME deferred, DecisionEngine zero-coupling); source selection and market cap unresolved pending SI-F0.1 feasibility spike; no production code, schema, or vendor added; SI-F0.1/F1 and SI-N0 not started |
| SI-F0.1 | Fundamentals Source Feasibility Spike — empirical live-feed interrogation across 8-company matrix (INFY, TCS, RELIANCE, PAYTM, IDEA, SCHAEFFLER, VBL, HDFCBANK); proven exact dissemination timestamps, semantic context classification (replacing context-ID assumptions), revision observation retention, and IndAS XBRL XML accessibility; commercial aggregators categorized with confidence discipline (PIT capabilities NOT VERIFIED / NOT PROVEN); source time vs market knowledge modeled; VBL split ISIN investigation establishes issuer vs security identity decoupling; 7/7 acceptance criteria passed | ✅ **COMPLETE AND FROZEN 2026-09-15** — finalized empirical report in `docs/research/SI-F0.1-FUNDAMENTALS-SOURCE-FEASIBILITY-SPIKE.md`; bounded SI-F1 scope approved; no production code, schema, or vendor added; SI-F1 and SI-N0 not started |
| SI-F1 | Fundamentals Identity + Immutable Source-Evidence Persistence Foundation — production persistence and domain foundation; decoupled corporate enterprise (IssuerRecord) from time-versioned security intervals (SecurityIdentity without stored is_active); immutable source filing observations (FundamentalFiling) with authentic publication precision and no timestamp fabrication; raw filing documents (FilingDocument) with original-byte SHA-256 integrity digests, bounded zlib compression with EOF verification, and 20 MB bounded size cap; schema version 21 DDL & migration; PIT-safe retrieval (list_filings_as_of); 26 focused tests + 87 repository regression tests passed; report in `docs/research/SI-F1-FUNDAMENTALS-EVIDENCE-PERSISTENCE-IMPLEMENTATION.md`; no normalized facts, derived metrics, market cap, crawler, or UI (SI-F2/F3 deferred) | ✅ **COMPLETE AND FROZEN 2026-09-15** |
| SI-F2A | Financial Fact Normalization Methodology & XBRL Mapping Discovery — empirical discovery across sanitized official exchange XBRL XML fixtures; two-layer architecture (raw source fact observation + canonical mapping); raw source occurrence identity `(document_id, source_occurrence_ordinal)`; period-safe duration identity (`period_start` + `period_end`); reported-facts-only normalization with synthetic quarter derivation prohibited; dimension preservation; deterministic duplicate policy; versioned PIT-safe mappings; non-financial mainboard corporate boundary (banking excluded empirically, NBFC/insurance conservatively excluded); initial canonical promotion bounded to 5 pure income concepts (`REVENUE_FROM_OPERATIONS`, `PROFIT_LOSS_BEFORE_TAX`, `PROFIT_LOSS_FOR_PERIOD`, `EPS_BASIC`, `EPS_DILUTED`); 2 capital concepts raw-mapped pending context promotion rules; secure parser requirement (`defusedxml.ElementTree`); machine-readable evidence artifact; no production code, schema remains v21 | ✅ **COMPLETE AND FROZEN 2026-09-15** — Owner / Chief Architect final review passed; decisions D1 through D12 approved; SI-F2B not started |
| SI-F2B | Production XBRL Raw Fact Extraction + Canonical Fact Persistence — production normalization pipeline transforming immutable SI-F1 XBRL filings into Layer 1 (`raw_financial_facts`) and Layer 2 (`canonical_financial_facts` + `canonical_fact_raw_sources`); strictly enforces frozen decisions D1-D12 (5 approved concepts only: `REVENUE_FROM_OPERATIONS`, `PROFIT_LOSS_BEFORE_TAX`, `PROFIT_LOSS_FOR_PERIOD`, `EPS_BASIC`, `EPS_DILUTED`; zero synthetic quarter derivation; capital guardrails intact; zero share count inference); secure XML parsing via `defusedxml` with 20 MB bounded size cap; multi-tier duplicate resolution strictly adhering to frozen D6 (identical Decimal values emit `EXACT_DUPLICATE`; no highest-precision winner; conflicting values quarantined with zero canonical promotion); approved `IN_BSE_FIN_2020_NS` taxonomy mapping; fail-closed `IssuerEligibilityResolver` protocol; schema version 22 DDL & migration; document-scoped raw `(document_id, ordinal)` and canonical `(document_id, canonical_concept, ...)` uniqueness; full 23-column semantic raw fact idempotency and canonical source link idempotency; typed dimensions preservation and canonical blocking; Clark notation raw QNames; exact source dissemination PIT retrieval (`publication_precision == EXACT_TIMESTAMP` and `source_published_at <= as_of`); 71 focused tests (11 parser, 8 mapping, 13 normalization service, 39 repository) + 4,178 repository regression tests passed; report in `docs/research/SI-F2B-PRODUCTION-FACT-NORMALIZATION-IMPLEMENTATION.md`; no fleet crawling, UI, or DecisionEngine coupling | ✅ **COMPLETE AND FROZEN 2026-09-16** — Owner / Chief Architect final review passed; Schema v22, raw fact extraction, canonical fact persistence, deterministic duplicate resolution, and exact source dissemination PIT retrieval frozen; SI-F2C, SI-F3, and SI-N0 not started |
...
| PORTFOLIO-INTELLIGENCE-V2 | Portfolio Intelligence V2 — one-day final Portfolio closure sprint: new `PortfolioStructuralReviewEngine` (candidate generation via confirmed D1 swing-point detection/clustering, separated from owner-facing structural selection — the PS-P10C.1 "too dense" failure mode deliberately avoided) producing Support 1, Major Support/Invalidation (structure or SuperTrend-anchored), Review Trigger (breakout or reclaim), Target 1-3, EXIT_RISK, and deterministic structural guidance; additive only via a new `structural_review` DTO/row field — Status/Conviction/Trend/Setup/Daily Review v0/Next Action/TradePlan/SuperTrend/DecisionEngine/ScoringEngine all unchanged; wired into the existing V1 holding-detail drawer only (no main-table change, no new milestone chain) | ✅ Owner/Chief Architect approved and frozen 2026-09-06 (implementation `d80139a`, owner correction round `8e7cdaa`: session-distance semantics fix, EXIT_RISK wording correction, and a genuine Major-Support fallback bug found/fixed via the mandatory historical PIT robustness replay — 12,075 real observations, same 20 holdings PS-P10C.1 used, Support 1 churn 3,571 vs PS-P10C.1's best-rejected-candidate 4,840 at higher population, zero duplicate/wide-zone pathologies, bounded staleness, clean determinism/future-leakage, golden result unchanged, full suite 3705 passed) — `portfolio-structural-review-v1` FROZEN. **PORTFOLIO TRACK CLOSED** (Portfolio Sync V1 + Portfolio Daily Review v0 + My Portfolio V1 UX Closure + Portfolio Intelligence V2); no V2A/V2B/V2C/PS-P10E authorized |
| MY-PORTFOLIO-HOLDINGS-CRUD | My Portfolio Holdings Edit/Delete — Owner-authorized narrow addition after Portfolio track closure (not a reopening of V2 methodology): per-holding Edit/Delete actions on the current-holdings list; edit corrects quantity/avg price only (source import provenance untouched, previous values recorded), delete removes the holding; both reuse the existing full Portfolio Sync pipeline to recalculate and reload the list/page (no isolated single-holding recompute exists) | 🔄 Implementation complete 2026-09-06, self-validated (full suite 3712 passed, ruff/mypy/git diff --check clean, live-verified against a real running server on a disposable scratch DB), committed as `b19910c`; ready for Owner/Chief Architect review |
...
| MY-PORTFOLIO-NEXT-LEVEL-UX-PLAN | My Portfolio next-level UX planning and handoff — document a milestone-based roadmap covering all 14 proposed UX upgrades: Portfolio Command Dashboard, Action Queue View, Column Profiles, Portfolio Heatmap, Symbol Detail Review Timeline, Change Since Last Sync, Risk Concentration Panel, Watchlist / Opportunity Bridge, Notes / Owner Override Layer, Review Session Mode, Pinned Rows, Smart Filters, Inline Mini Sparklines, and Export Profiles; add an AI-agent handoff with guardrails, implementation surfaces, non-goals, test expectations, coverage checklist, and recommended next authorization | ✅ Owner/Chief Architect approved and closed 2026-09-08 — plan executed through MP-NX6B; Watchlist / Opportunity Bridge skipped; Inline Mini Sparklines not shipped |
| MP-NX1 | My Portfolio Action Queue and Smart Filters — presentation-only morning triage over already loaded holdings/snapshot fields: Portfolio Command Dashboard attention counts, Action Queue list scope, and Smart Filters for Status / Daily Review / Next Action / Trend / Setup / P&L state / currentness / evidence availability; Near Trigger / Near Support / Fresh Breakout remain unavailable pending owner-approved threshold semantics; no methodology, interpretation-version, export, sync, or privacy-contract change | ✅ Owner/Chief Architect approved and closed 2026-09-07 |
| MP-NX2 | My Portfolio Change Since Last Sync and Risk Concentration — display-only latest-vs-previous snapshot diffs (no schema change) with factual row badges and a detail-overlay Since last sync panel, plus a client-side Risk Concentration Panel aggregating existing Status / Trend / Next Action / conviction / value / P&L fields; no new risk score, no interpretation-version bump, no methodology change | ✅ Owner/Chief Architect approved and closed 2026-09-07 |
| MP-NX3 | My Portfolio Column Profiles — presentation-only table profiles (Compact Scan, P&L Review, Technical Review, Risk Review, Full Audit) controlling visible columns, default sort, and density; owner-pinned rows kept visible across sort/filter and labeled as owner-pinned; export profiles Daily Review / Full Audit / Private Sharing on the existing selected-column export contract; localStorage only; no methodology, interpretation-version, or schema change | ✅ Owner/Chief Architect approved and closed 2026-09-08 |
| MP-NX4 | My Portfolio Heatmap — presentation-only snapshot tiles sized by current value or investment and colored by existing P&L % / Status / Daily Review / Trend / Next Action fields; tile click opens the existing detail overlay; privacy uses equal tile size and mutes P&L % color; no inline sparklines because snapshot rows have no D1 close series; no new risk score, methodology, interpretation-version, or schema change | ✅ Owner/Chief Architect approved and closed 2026-09-08 |
| MP-NX5 | My Portfolio Symbol Review Timeline — display-only per-holding timeline from recent SUCCESS/PARTIAL snapshots using existing snapshot_diff fields (price, P&L %, Status, Trend, Daily Review, Next Action, guidance); partial syncs marked honestly; privacy masking reused; Watchlist / Opportunity Bridge skipped because no stable My Portfolio opportunity contract exists and no new ranking was authorized; no methodology, interpretation-version, or schema change | ✅ Owner/Chief Architect approved and closed 2026-09-08 |
| MP-NX6 | My Portfolio Owner Notes — persist owner-authored thesis / watch condition / reminder / review comment / follow-up on current holdings in `portfolio_holding_notes`; visually and contractually separate from ATHENA evidence; notes never change Status, scores, or guidance; empty notes are deleted; notes follow NSE→BSE remaps (dest note wins on conflict) and are removed with the holding, confirm REMOVED, and My Portfolio reset | ✅ Owner/Chief Architect approved and closed 2026-09-08 |
...
| MP-RV2 | Grouped sticky header actions and section navigation; measured section landing clearance and tab-switch refresh; three-level asset cache busting; root-caused and closed an exposed 32px scrollport top gutter above the opaque header (moved that spacing from the active Portfolio viewport into scrolling workstation content, so it scrolls away instead of remaining a permanent hole the sticky header never covers) — see the full correction history in IMPLEMENTATION_SUMMARY.md and the revamp design document | ✅ Owner/Chief Architect approved and closed 2026-09-12, confirmed on the real production dashboard |
| MP-RV3 | Clearer copy, in-place filtering and explicit View Holdings; reference-mock triage with production screenshot contrast/alignment corrections, complete Daily Review choices and distinct Trend/Setup labels; calculation/filter predicates unchanged | ✅ Owner approved and closed 2026-09-12; desktop fixture screenshot, behavior test, JS syntax and new-test lint pass; full suite 4018 passed, one existing macOS launcher failure; authenticated/mobile UI verification pending |
| MP-RV4 | Collapse up-to-four independent per-row change badges into one compact indicator with the detail reachable on hover; pin/note badges stay separate (owner-authored state, per MP-NX6's separation guardrail) | ✅ Owner approved and closed 2026-09-12; 4019 passed, one existing launcher failure; reason-preservation test, lint and JS syntax pass; live UI verification pending |
| MP-RV5 | Owner-expanded Risk & Heatmap reference layout: combined workspace, wider top holdings, conviction chips, categorical/rank styling and readable tile-tone labels; existing controls, rank limits, privacy and weighted sizing preserved | ✅ Owner approved and closed 2026-09-13 via continue authorization; known validation limits retained |
| MP-RV6 | Owner-requested mock-inspired typography and visual consistency across the complete My Portfolio page and overlays; portfolio-scoped styling only, existing functionality/privacy and other tabs unchanged | ✅ Owner approved 2026-09-13; remaining browser validation assigned to MP-RV7 |
| MP-RV7 | Isolated populated-browser validation and responsive hardening; preserve desktop sticky behavior, make narrow layouts usable and keep export within the viewport | Implemented for Owner review 2026-09-13; 4022 pytest passes, one opt-in skip, existing launcher failure retained; see docs/design/MY-PORTFOLIO-RV7-VALIDATION.md |
...
| ID-7F2 | Entry Actionability production activation + first canonical canary — safely migrate the canonical DB schema v17→v18, activate the already-closed ID-7E canonical stage through the normal production runtime path, and observe the first genuine canonical-cycle canary. Operational activation only — no methodology/threshold/schema-design change, no backfill, no Mode-B shadow equivalence | 🔄 **ID-7F2 PRE-ACTIVATION PREPARATION COMPLETE. PRODUCTION ACTIVATION DEFERRED TO THE NEXT NSE TRADING-DAY, OWNER-OPERATED RESTART. ID-7F2 REMAINS OPEN** (2026-09-05) — Pre-activation safety check (read-only): canonical `db/athena.db` confirmed still `schema_version=17`, WAL mode, no `RUNNING` cycle (last: `run-closing-20260904T154545` COMPLETED), git HEAD at the last committed ID-7F1.1 work. Migration-source audit: `git show` on the ID-7A commit confirms schema v17→v18 introduces *only* the already owner-approved `entry_actionabilities` table + 2 indexes (82 insertions, nothing else; ID-7A.1/ID-7A.2 touched `schema.py` not at all) — migration mechanism is the existing idempotent `SqliteRepository.initialize()`, called unconditionally on every `athena-serve` startup; no ad-hoc SQL needed or run. Pre-migration backup created and verified using the same safe read-only-source + SQLite-online-backup-API pattern ID-6E.2 established: `db/backups/athena-pre-id7f2-schema-v18-activation-20260905T064233Z.db` (5,166,485,504 bytes, `integrity_check: ok`, 0 FK violations, `schema_version: 17`, full per-table record counts captured for post-migration comparison) — retained, not restored. 644 pre-activation focused tests passed (data_layer + ID-7E workflow). **Live-process restart was attempted and correctly blocked by the auto-mode safety classifier** (ending/relaunching a real running service is a hard-to-reverse action requiring explicit owner execution, not autonomous action) — reported to the owner rather than worked around. Separately confirmed, via the real `CalendarEngine`, that 2026-09-05 is `SessionType.WEEKEND` — no scheduled cycle would fire today regardless, so deferring activation costs nothing. **Owner decision: proceed with NO live-process action today; do not weaken/bypass auto-mode; restart is an explicit owner-operated action at the next trading-day window.** Exact minimal stop/restart/verification procedure (A–G) documented for the owner to execute themselves: `docs/research/ID-7F2-ENTRY-ACTIONABILITY-PRODUCTION-ACTIVATION-CANARY.md`. Zero code/schema/methodology change; zero live-process action; zero manual cycle/Validate-All; zero EMR/DarvaX touch; zero ID-7F1 modification. Not marked Owner-approved. **Addendum (2026-09-07, read-only activation-provenance forensics + first canonical canary):** on resuming ID-7F2 the next trading day, the fresh pre-flight instead found `schema_version=18` and a real, non-empty `entry_actionabilities` table — the activation boundary had already been crossed without any owner-operated restart having been reported. Per explicit owner instruction, no restart/migration was attempted; a read-only forensic investigation was authorized instead. **Activation-provenance classification: `EXECV_RESTART_CONFIRMED`** — `artifacts/logs/athena-serve.log` contains direct, repeated evidence of the in-place `os.execv` restart mechanism (`POST /api/v1/ops/restart` immediately followed by `Started server process [same PID]`, no intervening shutdown lines); PID 2453 itself shows 11 such in-place restarts. Exact instant not recoverable (no per-line timestamps in the log); bounded instead: `activation_lower_bound = 2026-09-05T11:00:15+05:30` (ID-7E's own commit `684ec4b`), `activation_upper_bound = 2026-09-06T10:23:07 UTC` (the earliest real persisted EA row's `evaluated_at`/`persisted_at`). **The 2026-09-04 `as_of` / 2026-09-06 `persisted_at` gap (2 of 228 rows, NSE:GOLDBEES/NSE:SILVERCASE) is fully traced, not inferred:** both rows' `run_id`s carry `config_snapshot_id='cfg-symbol-validate'`, sourced to exactly one call site, `src/athena/ops/symbol_validate.py:243` — a pre-existing (651 historical runs since 2026-07-24), on-demand, dashboard/CLI single-symbol "Validate" feature that reuses the same `OwnerValidationPipeline` ID-7E's stage was wired into; classified **`OTHER_EXPLAINED_PRODUCTION_PATH`**, historical-backfill verdict **`HISTORICAL_BACKFILL_ABSENT`** (only 2/228 rows via this path; no code exists that iterates the 12,214-row EQ history and writes `entry_actionabilities`; the remaining 226/228 rows all trace to one genuine scheduled `PREMARKET` cycle today). Schema-v18 structure verified exactly matching ID-7A's approved design (23 columns, composite 7-column PK, 2 indexes); `integrity_check: ok`, 0 FK violations; every pre-existing table-count delta vs. the frozen pre-migration backup is expected/mutually coherent (e.g. `entry_qualifications` +228 exactly equals new EA rows; `runs` +3 exactly equals 2 symbol-validate + 1 premarket) — zero unexpected loss. Full 228-row inventory: 0 out-of-scope rows, 0 exact-binding defects, 0 duplicate/conflicting identities, 0 WATCH invariant violations (all 228 are WATCH/`NOT_ACTIONABLE` with both frozen upstream reason codes), 0 TRADE rows (`TRADE_EMPIRICAL_CANARY_NOT_AVAILABLE`). **First eligible post-activation canonical canary frozen:** `run-premarket-20260907T081534` (`2026-09-07-premarket`, PREMARKET phase, `cfg-host-ops`, 385 total decisions: 226 WATCH/159 NO_TRADE/0 TRADE) — expected in-scope EA population 226, actual 226, 0 missing, 0 unexpected, 0 binding defects. Zero actionability-stage-attributable provider calls (source-confirmed); zero persisted `CURRENT`/`STALE`/`SUPERSEDED`/`SESSION_CLOSED` values (currentness remains exclusively read-time). Acceptance verdict: **`ACTIVATION_PATH_PARTIALLY_UNPROVEN_BUT_RUNTIME_ACTIVATION_VERIFIED`** (the `execv` restart mechanism is directly proven; the exact caller/operator behind each `POST /api/v1/ops/restart` is not recoverable from available logs) — all other frozen acceptance conditions met. **Status: ID-7F2 ACTIVATION + CANARY VERIFIED — READY FOR OWNER / CHIEF ARCHITECT REVIEW.** No restart/migration/repair/data modification performed; PID 2453 and every persisted row left untouched. Mode-B (ID-7F3), EMR, and DarvaX untouched. **Owner/Chief Architect decision (2026-09-07): ID-7F2 OWNER APPROVED / CLOSED.** All findings above accepted as final: schema v18 verified, `EXECV_RESTART_CONFIRMED` from direct log evidence, exact restart-caller identity accepted as unrecoverable (`ACTIVATION_PATH_PARTIALLY_UNPROVEN_BUT_RUNTIME_ACTIVATION_VERIFIED`), `HISTORICAL_BACKFILL_ABSENT`, the 2 historical-as_of rows fully explained by the pre-existing `symbol_validate` path, 228/228 (at the time of review) production rows structurally coherent with 0 out-of-scope/binding/duplicate defects and exact WATCH semantics, the first genuine canary (`run-premarket-20260907T081534`) matching its expected population exactly (226/226, 0 missing/unexpected/binding-defects), zero EntryActionability-attributable provider calls, DB integrity healthy, backup preserved, and the absence of TRADE/ACTIONABLE/UNKNOWN/SHORT empirical examples at that time explicitly not treated as a blocker. No ID-7F2.x correction milestone introduced; the `symbol_validate` interaction is recorded as an accepted architectural observation, not a defect. **ID-7F3 (Mode-B production-vs-replay shadow equivalence) authorized same day — see row below.** |
| ID-7F3 | Entry Actionability production-vs-replay Mode B shadow equivalence — independently reconstruct real persisted production `EntryActionability` observations using the frozen ID-7F0 Mode-B contract and compare field-by-field against the production-persisted artifact. Deterministic behavioral equivalence validation only — no methodology tuning, no profitability analysis, no currentness validation, no backfill, no second live evaluator | ✅ **ID-7F3 MODE-B PRODUCTION EQUIVALENCE EXACT — READY FOR OWNER / CHIEF ARCHITECT REVIEW** (2026-09-07) — Added `run_shadow_equivalence` to the existing `src/athena/data/id7f1_entry_actionability_replay.py` (reusing Mode A's `ReadOnlyStore`/`_get_decision`/`_reconstruct_market_evidence`/`_validate_binding` helpers directly, per the authorization's own "reuse, do not duplicate" instruction) rather than a new module. For every persisted production `entry_actionabilities` row as of a frozen `validation_cutoff` (captured before the population query runs, so a live natural cycle ticking mid-audit cannot create a moving denominator): resolves the exact Decision (`decision_id`) and exact upstream `EntryQualification` (full 5-field composite identity) the persisted row itself names — never "latest" of either; reconstructs the same bounded point-in-time evidence (M5/VWAP/OR15) ID-7E's production stage would have supplied, using the same canonical helpers; runs the real, unmodified `EntryActionabilityEngine` with a fixed diagnostic `evaluated_at` (methodology cannot see wall-clock time); evaluates twice independently for determinism; compares the reconstructed result against the persisted production artifact on the frozen 19-field comparison mask (`athena.data.store.repository._entry_actionability_payload`'s own frozen field list, excluding only `run_id`/`cycle_id`/`evaluated_at`/`persisted_at`) with a most-specific-first mismatch classifier (`PROVENANCE_MISMATCH`/`STATE_MISMATCH`/`REASON_MISMATCH`/`VALUE_OBJECT_MISMATCH` — denormalized upstream-context differences, i.e. `decision_type`/`direction`/`entry_qualification_state`, are checked and classified `PROVENANCE_MISMATCH` before the derived `state` field, so a real mismatch there is never silently reported as a mere state disagreement). Reports scheduled-cycle vs. `symbol_validate`-path equivalence separately (descriptive provenance only — one frozen evaluator contract for both) via a `runs.config_snapshot_id` join. 23 new focused tests (comparison-mask exclusion proofs, all 4 mismatch-classification categories including the context-vs-state ordering proof, a genuine mutation/negative proof that a corrupted persisted row is caught as a real `REASON_MISMATCH` rather than a false EXACT_MATCH, path-classification, validation-cutoff freeze-semantics, naive-cutoff/naive-evaluated_at rejection, never-mutates-source-DB, schema-unchanged, zero-`save_entry_actionability`-call source-scan, and infrastructure-failure-propagates). Full suite: **3736 passed, 1 pre-existing unrelated skip, 0 failures** (was 3644 at ID-7F1.1's own close; the delta also reflects unrelated Portfolio/EMR work committed between milestones). **Run against the real, live `db/athena.db` (strict read-only, zero writes, zero provider calls), frozen at `validation_cutoff=2026-09-07T04:15:24 UTC`: 488/488 production rows reconstructed successfully, 0 binding defects, 0 PIT-evidence defects, 0 unexpected exceptions, 0 determinism mismatches, 0 production-equivalence mismatches — 100% EXACT_MATCH (488/488), both paths exact (486/486 scheduled-cycle, 2/2 `symbol_validate`), `mode_b_acceptance: true`, schema unchanged 18→18.** New empirical coverage obtained naturally during this milestone (the production system kept running live cycles throughout, exactly the "moving denominator" the frozen cutoff exists to control for) — for the first time in the entire ID-7 track: genuine `TRADE_QUALIFIED_AVAILABLE` (15 real TRADE+QUALIFIED observations) and genuine `UNKNOWN_AVAILABLE` (9 real EA rows) now exist; `ACTIONABLE`/`LONG` remain `NOT_AVAILABLE` (all real TRADE decisions observed to date are SHORT — the previously-documented EQ long-bias limitation is unrelated and still stands, `LONG_VALIDATED_SHORT_UNVALIDATED` unchanged, since it concerns the *EQ* formula's own directional bias, not EntryActionability's evaluation of whichever direction a Decision carries). All new TRADE_QUALIFIED observations reconstructed to the exact same `UNKNOWN` verdict as production, with 0 mismatches. Zero production writes; zero schema/service/methodology change; zero EMR/DarvaX touch. `git diff --check` clean; diff scoped to exactly the replay module and its test file. Full report: `docs/research/ID-7F3-ENTRY-ACTIONABILITY-PRODUCTION-VS-REPLAY-MODE-B-SHADOW-EQUIVALENCE.md`. **Owner/Chief Architect decision (2026-09-07): ID-7F3 OWNER APPROVED / CLOSED — classification `MODE_B_PRODUCTION_EQUIVALENCE_EXACT` accepted** (488/488 EXACT_MATCH, both paths exact, 0 defects of any kind, 15 TRADE+QUALIFIED and 9 UNKNOWN observations newly available, SHORT production evidence now available — `ACTIONABLE`/LONG production examples still not naturally observed, recorded as an empirical-coverage limitation that does not block closure; `LONG_VALIDATED_SHORT_UNVALIDATED` stands unchanged for the upstream EQ methodology, unaffected by the newly-observed SHORT EntryActionability population). No ID-7F3.1 correction milestone; no periodic shadow execution added to production; natural accumulation continues and `run_shadow_equivalence` may be re-invoked later ad hoc. **Owner/Chief Architect decision, same day: ID-7 OVERALL OWNER APPROVED / CLOSED** — the entire ADR-015/ID-7A0 through ID-7F3 track (architecture, domain model, deterministic evaluator, canonical workflow integration, historical replay, production activation, and production-vs-replay shadow equivalence) is now closed. Same day, Owner/Chief Architect authorized **ID-8** (entry/risk methodology validation) |
| ID-8 | Entry/risk methodology validation — given an actionable/eligible intraday opportunity, what entry/risk structure is empirically defensible (entry timing, invalidation, MFE/MAE, target reachability, time-to-event, RR geometry)? Discovery + one evidence-grounded empirical contract only — no threshold freeze, no ID-9/10/11 work, no methodology mutation | 🔄 **ID-8 DISCOVERY + EMPIRICAL METHODOLOGY CONTRACT READY FOR OWNER / CHIEF ARCHITECT REVIEW** (2026-09-07) — Read-only discovery found: (1) `docs/research/ID-7B1-RETROSPECTIVE-TRADE-EQ-RECONSTRUCTION.md` §16-20 already computed MFE/MAE, T1(+1%)/T2(+1.5%) hit rates and timing, 3 invalidation candidates head-to-head (VWAP-loss/OR15-boundary/D1-ATR), and RR distributions on a real 783-episode LONG-only cohort — never packaged as its own milestone and its harness was never committed (`docs/research/ID-7B1-...md` §25: scripts lived in a session scratchpad, not `src/`), so it must be re-derived, not re-run, though its numeric findings are directly reusable evidence; (2) a fresh, natively-persisted TRADE+QUALIFIED population exists with zero replay needed — 60 real `entry_qualifications` rows (`state='QUALIFIED'`, `decision_type='TRADE'`), all from today (2026-09-07), the first real SHORT TRADE decisions in ATHENA's history (417 real SHORT TRADE decisions total today, versus 96,985 LONG across 20 earlier sessions, 2026-07-31→2026-08-27 — zero SHORT decisions of any kind existed before today); (3) a newly-discovered, real mechanistic explanation for the continuing zero-`ACTIONABLE` finding — **all 60 of today's real SHORT TRADE+QUALIFIED rows resolved to `UNKNOWN`/`INVALIDATION_UNAVAILABLE`, 60/60 (100%)** — sharpening the known `LONG_VALIDATED_SHORT_UNVALIDATED` limitation into a concrete mechanism: EQ v0's "QUALIFIED" criterion is bullish-shaped (VWAP-positive AND BULLISH trend), so a SHORT Decision that happens to receive a QUALIFIED EQ verdict inherits an entry-side VWAP relation on the structurally wrong side for a SHORT's own risk geometry — reported honestly as an ID-6/EQ methodology observation, explicitly not fixed here (out of ID-8's authorized boundary). Defined the full empirical research contract: observation unit = episode-first-checkpoint (reusing ID-7B.1's own zero-invented-parameter episode-boundary rule); entry proxy = frozen ID-7 checkpoint-close, explicitly labeled `CHECKPOINT_CLOSE_THEORETICAL` (no execution-cost data exists anywhere in the schema — confirmed via a full 30-table scan; `quotes` has no bid/ask/spread, `trade_outcomes` exists but holds exactly 1 row); leak-safe same-session forward window (direct SQL date/timestamp filter, never `LIMIT`, mirroring ID-7B.1's own §17 safeguard); direction-aware MFE/MAE as running-max over candle high/low; intrabar-touch vs. close-confirmed target-hit semantics (both reported); a new, mandatory `AMBIGUOUS_SAME_BAR` same-candle target/stop-ordering policy (a genuine gap confirmed absent from ID-7B.1's own report by direct text search); a 6-label outcome taxonomy separating reachability from first-terminal-outcome; session-grouped chronological statistical design (never random-split, leave-some-sessions-out recommended for a future full-scale pass, reusing ID-7B.1's own §10 recommendation); LONG/SHORT reported strictly separately, never pooled. **A small read-only feasibility sample** (all 60 real native TRADE+QUALIFIED rows, zero writes, zero provider calls) proved the mechanics work end-to-end on real data: 60/60 reconstructed (0 skips), T1 hit rate 21/60 (35.0%, median 34.0 min to hit), T2 hit rate 8/60 (13.3%), median MFE 0.61%/MAE 0.63%, 0 `AMBIGUOUS_SAME_BAR` occurrences — explicitly reported as a **mechanical proof only**, never methodology-validating evidence, since every one of these 60 checkpoints already carries invalid VWAP-based risk geometry (§46), so the invalidation leg of the contract could not be exercised end-to-end on this batch (deferred to a future continuation against ID-7B.1's valid-geometry LONG cohort). Zero source/schema/methodology/EMR/DarvaX changes; zero production writes/provider calls; `db/athena.db` confirmed `integrity_check: ok`, `schema_version` unchanged at 18 before and after. Full report: `docs/research/ID-8-ENTRY-RISK-METHODOLOGY-VALIDATION-DISCOVERY.md`. **Owner/Chief Architect decision (2026-09-07): ID-8 Discovery + Empirical Methodology Contract OWNER APPROVED / CLOSED — the contract is FROZEN for this validation stage.** Owner decisions on the three open questions: (1) YES — rebuild the historical LONG replay cohort; (2) YES — exercise forward VWAP-loss end-to-end on that valid-geometry LONG population; (3) NO — do not interrupt ID-8 to modify/investigate ID-6 SHORT methodology now. Two small consistency-cleanup rounds were applied to the discovery report before freeze (target-touch direction wording, same-bar ordering tightened to `TARGET_NO_LATER_THAN_INVALIDATION`, taxonomy split into reachability metrics vs. terminal ordering) — zero numeric results changed by either round. **ID-8 (full historical study) authorized same day, completed same day: ID-8 FULL HISTORICAL ENTRY/RISK OUTCOME VALIDATION READY FOR OWNER / CHIEF ARCHITECT REVIEW.** New committed module `src/athena/data/id8_entry_risk_outcome_validation.py` independently reconstructs (never copies) the real historical LONG population using two structurally separated phases (PHASE A point-in-time observation selection, reusing `id6e_replay_shadow_validation.run_replay`'s own exact evidence-composition pattern generalized to an arbitrary Decision/checkpoint; PHASE B forward outcome analysis, genuinely new code — no forward-reading capability existed anywhere in the repo before this). Episode construction found and fixed a real bug during self-validation: an earlier draft pre-filtered to `decision_type='TRADE'` before grouping, silently discarding the intervening-non-TRADE rows needed to detect a true episode boundary, over-merging episodes — fixed by reading the full per-instrument decision timeline first. After the fix, population-identity statistics matched ID-7B.1's own published numbers **exactly**: 6,624 LONG episodes, 783 REPLAYED_QUALIFIED LONG observations, max episode length 60 (`NSE:PAYTM`, 2026-08-10) — all `EXACT_MATCH`. However, several outcome statistics (T1 hit rate 31.67% vs. ID-7B.1's 23.88%; T2 18.77% vs. 14.81%; median MFE 0.604% vs. 0.434%) differ materially despite the identical population and could not be fully root-caused, since ID-7B.1's own harness was never committed (no line-by-line diff possible) — classified `UNEXPLAINED_DIFFERENCE` honestly rather than forced to match; **per the authorization's own instruction, this new committed harness and its numbers are now authoritative for future ID-8 work**, without overwriting or invalidating ID-7B.1's own preserved report. Real findings: VWAP-loss occurred in 66.8% of 756 valid-geometry observations (median time-to-loss 40 min); terminal ordering 52.5% `INVALIDATION_FIRST` vs. 24.7% `TARGET_SIDE_NO_LATER_THAN_INVALIDATION` vs. 19.4% `SESSION_END_NO_RESOLUTION` vs. 3.4% `INSUFFICIENT_FORWARD_DATA`; zero observations ever classified `AMBIGUOUS_SAME_BAR` (structurally proven unreachable for the target-vs-VWAP-loss pairing, per the frozen contract); a strong, real, monotonic session-time-of-entry association with T1 reachability (44.3% at 09h falling to 4.7% at 15h) classified `EXTENSION_EFFECT_WARRANTS_CALIBRATION`; RS and risk-distance quartiles both show real monotonic-ish positive associations with T1 reachability. Native SHORT population grew to 67 real rows (from 60 at discovery) — still 100% `UNKNOWN`/`INVALIDATION_UNAVAILABLE`, reported strictly separately, never pooled with LONG; `LONG_VALIDATED_SHORT_UNVALIDATED` unchanged; zero ID-6/EQ methodology touched, per the Owner's own explicit decision. Determinism proven: an independent second full run reproduced the exact same 783 LONG observation identities with byte-for-byte identical full records (0 mismatches) — only the live, still-growing native SHORT snapshot varied between runs, expected and explained. 12 new focused tests (PIT/forward structural isolation, episode-boundary bug-fix regression, direction-correct target-touch proof, no-provider/no-persistence source scans). Full suite: **3756 passed, 1 pre-existing unrelated skip, 0 failures**. Zero schema/production/EMR/DarvaX changes; `db/athena.db` confirmed unchanged throughout (schema 18, integrity ok). Classification: **`ID8_V0_ENTRY_RISK_PARTIALLY_SUPPORTED`** — real, mechanically sound evidence at scale, but open threads (the outcome-rate discrepancy, incomplete OR15/D1-ATR comparators, an unisolated extension effect) mean V0 is not yet frozen. **Owner/Chief Architect source review found several correctness/completeness defects, corrected in place inside this same milestone (no ID-8.1/ID-8.x created):** (1) LONG and SHORT were being pooled into one 794-observation denominator despite the report claiming otherwise — `_summarize` now produces two entirely independent blocks, `primary_LONG` (783 observations, the only block used for methodology evidence) and `replayed_SHORT_diagnostic` (12 observations, 100% invalid geometry, never pooled); (2) episode reconciliation now computed LONG-only (96,985 decisions/6,624 episodes/14.64 mean/60 max — all `EXACT_MATCH` to ID-7B.1), not a combined-population figure; (3) the forward window now bounded by the canonical session close (`CalendarEngine.context_for` + `session_open_close_ts`), not a same-date-only filter; (4) entry selection now uses the canonical `latest_completed_candle` helper, removing a raw-SQL semantic fork; (5) terminal ordering now requires valid initial VWAP geometry — an invalid-geometry observation reaching T1 correctly reports `terminal=None`, never a false ordering claim (proven both for LONG, where `invalid_geometry_n=0`, and for the SHORT diagnostic, where `invalid_geometry_n=12/12` and the terminal-ordering denominator is correctly 0); (6) VWAP-loss semantics resolved with a direct frozen-source citation (`ID-7B-ENTRY-RISK-METHODOLOGY.md:303-304`: "closes back through session VWAP ... on a completed M5 bar") confirming the evolving, session-cumulative interpretation, not a level frozen at entry; (7) MFE-before-VWAP-loss, MAE-before-T1, MAE-before-T2 implemented; (8) OR15 comparator completed (22.64% event rate, 88.24% before T1, 0 `AMBIGUOUS_SAME_BAR` occurrences, kept structurally separate from VWAP-loss's own close-confirmed-only semantics); (9) D1-ATR(1×) comparator implemented (1.72% event rate — `EXACT_MATCH` to ID-7B.1's own 1.76%); (10) regime association implemented via the real, unmodified `RegimeEngine`, PIT-replayed; (11) a fixed-seed session-block bootstrap added for the primary LONG rates (T1 90% CI [28.81%, 36.69%]; VWAP-loss 90% CI [56.77%, 75.92%]); (12) a descriptive first-half/second-half chronological stability view added (34.53%→32.08% T1 rate, stable, not concentrated in a few sessions); (13) the ID-7B.1 reconciliation re-run on the corrected LONG-only basis — population-identity metrics remain `EXACT_MATCH`, VWAP-loss and D1-ATR event rates now also reconcile almost exactly, but T1/T2/MFE/OR15-timing remain `UNEXPLAINED_DIFFERENCE`/`LEGACY_OUTCOME_DISCREPANCY_UNRESOLVABLE_FROM_AVAILABLE_SOURCE` (ID-7B.1's own harness was never committed); (14) the session-time association reclassified from `EXTENSION_EFFECT_WARRANTS_CALIBRATION` to `LATE_SESSION_TIME_BUDGET_EFFECT_OBSERVED`, since this pass does not isolate genuine extension/chase risk from ordinary remaining-session-time effects. Determinism re-proven on the corrected harness (byte-for-byte identical `primary_LONG` block across an independent second full run). 7 new regression tests (19 total, up from 12); full suite **3763 passed, 1 pre-existing unrelated skip, 0 failures**. Zero schema/production/EMR/DarvaX changes throughout. Classification unchanged: **`ID8_V0_ENTRY_RISK_PARTIALLY_SUPPORTED`**, now on materially more rigorous grounds. **Second correction round (2026-09-07, same day, same milestone, no ID-8.1/ID-8.x):** Owner source review found three further methodology-rigor gaps — (1) a real MAE-strictly-before-target intrabar-ordering bug (a same-bar co-occurring adverse extreme and target touch was incorrectly counted as "before" the touch; OHLC cannot prove that ordering) fixed by snapshotting the running tracker before folding in the hit bar's own value and freezing it on hit; (2) D1-ATR's assumed `CLOSE_CONFIRMED` event semantics investigated against frozen sources and found unstated anywhere (methodology doc, `TradePlan._build_plan`, ID-7B.1 §18's own uncommitted harness) — withdrawn, downgraded to level/geometry-only (`D1_ATR_LEVEL_GEOMETRY_RECONSTRUCTED_EVENT_SEMANTICS_NOT_RECOVERABLE`); (3) OR15's assumed `INTRABAR_TOUCH` semantics (circularly justified by ID-7B.1's own measured result) investigated and found explicitly contradicted by the methodology doc's own text ("never via breakout_event... semantics") — withdrawn, downgraded to level/geometry-only (`OR15_LEVEL_GEOMETRY_RECONSTRUCTED_EVENT_SEMANTICS_NOT_RECOVERABLE`). The flagged "19 vs. 20 sessions" bootstrap/chronological mismatch was investigated and found to be a narrative writing error (not a code/population defect — both always used the same 19-session population), with the bootstrap code additionally hardened to explicitly share the same forward-data-bearing session population regardless. VWAP-loss semantics re-examined per instruction and confirmed unchanged. 6 tests added/1 removed (24 total); full suite **3768 passed, 1 pre-existing unrelated skip, 0 failures**. Rerun against the real, unchanged `db/athena.db` (schema 18→18, PID 2453 untouched): LONG population fully reconciled, unaffected (783/756/0). Acceptance Question F revised to `NO_DISPLACEMENT_CLAIM_POSSIBLE_FROM_COMPARABLE_EVENT_EVIDENCE`. New §32 directly answers the Owner's own freeze question: **PRIMARY LONG can be frozen independently of the unresolved legacy T1/T2/MFE discrepancy** — every PRIMARY LONG dimension the Owner named is deterministic, sourced, and correct; the harness is judged sufficiently self-consistent and source-grounded to become authoritative. Full corrected report: `docs/research/ID-8-FULL-HISTORICAL-ENTRY-RISK-OUTCOME-VALIDATION.md`. **Third correction pass (2026-09-07, same day, same milestone, diagnostic-only, no ID-8.1/ID-8.x):** Owner accepted the methodology in substance, held closure for one narrow diagnostic sign defect — `t1_hit_bar_adverse_excursion_pct`/`t2_hit_bar_adverse_excursion_pct` could go negative (a signed displacement) when adverse excursion must be a non-negative magnitude by definition; fixed by clamping at `max(0, adv)`. 2 tests added (26 total); full suite 3770 passed. Rerun confirms every other PRIMARY LONG field byte-for-byte unchanged (schema 18→18, PID 2453 untouched); only the two corrected distributions changed (medians 0.0%/0.0%, down from negative). Classification/comparator verdicts (`ID8_V0_ENTRY_RISK_PARTIALLY_SUPPORTED`, `LONG_VALIDATED_SHORT_UNVALIDATED`, both `_NOT_RECOVERABLE` verdicts) preserved unchanged. Status **ID-8 PRIMARY LONG V0 ENTRY/RISK METHODOLOGY CORRECT AND READY FOR OWNER FREEZE**. **Owner/Chief Architect decision (2026-09-07): ID-8 PRIMARY LONG V0 ENTRY/RISK METHODOLOGY OWNER FROZEN, ID-8 OVERALL CLOSED.** Final evidence classification: `ID8_V0_ENTRY_RISK_PARTIALLY_SUPPORTED`. ID-9 (position sizing / capital allocation) authorized to start same day |

**ID-9 (2026-09-07): DISCOVERY + V0 CONTRACT READY FOR OWNER / CHIEF ARCHITECT REVIEW.** Read-only discovery only, no implementation, no schema/production change. Key findings: (1) `EntryActionability` (ID-7A, already persisted, schema v18) already carries the exact entry/risk fields ID-9 needs — `entry_reference.price`, `operative_invalidation.level`, `reward.t1_price`/`t2_price`/RR — so **no EntryRisk artifact gap exists**; ID-9 consumes `EntryActionability` (state==ACTIONABLE) directly, never a research harness. (2) A complete, tested P5.2-P5.6 capital-allocation/position-sizing/order-planning/broker-translation/execution pipeline already exists in the repo (`allocation/`, `sizing/`, `orders/`, `brokers/`, `execution/` engines, matching `config/*.json` policy files) — **but it is entirely dormant in production**: zero references from `cli.py`, `runtime/`, `ops/owner_validation.py`, or `scheduling/`; it operates on the pre-ID-track generic `Decision` object, never `EntryQualification`/`EntryActionability`; its own capital source is the separate legacy `owner_positions` manual-fill ledger, never a live feed. (3) `config/capital.json`/`CapitalConfig` and `config/risk.json`/`RiskConfig` (real-looking `total_capital`/`per_trade_risk_pct`/position-sector caps) are loaded by the config system but consumed by zero engines anywhere. (4) No live broker cash/margin/account-balance capability exists at all (zero grep hits in `brokers/`); every capital figure in the repo is either a hardcoded config default or manually entered (legacy fill ledger or My Portfolio CSV/XLSX import) — a V0 sizing engine must therefore be explicit `THEORETICAL_POLICY_CAPITAL_SIZING`, never implying brokerage sync. (5) NSE/BSE cash-equity lot size is always 1 in this codebase's data path (confirmed empirically), but **no tick-size price-rounding convention exists anywhere**, including in `TradePlan`. (6) Recommends **refusing SHORT sizing in V0** (Option A, not compute-and-label) given `LONG_VALIDATED_SHORT_UNVALIDATED`'s own zero-SHORT-observation evidence base. Proposes (not frozen): a `PositionSizing` V0 contract mirroring ADR-013/ADR-015's own identity/state/reason-family patterns, risk-quantity + max-value-quantity + theoretical-available-capital-quantity as the only V0 constraints (liquidity/concentration/portfolio-risk gates classified `NOT_AVAILABLE`/`CONTEXT_ONLY`/`OUT_OF_V0`), a `position_sizing` `WorkflowStage` (`depends_on=("entry_actionability",)`) mirroring ID-7E's own integration precedent, and a multi-slice implementation sequence (pure contract+evaluator first, persistence/workflow-wiring later) mirroring ID-7A→ID-7E's own staged approach. Zero code/schema/production changes; zero EMR/DarvaX touch; `git diff --check` clean (one new untracked report file only). Full report: `docs/research/ID-9-POSITION-SIZING-DISCOVERY-AND-V0-CONTRACT.md`. **Owner/Chief Architect decision (2026-09-07): ID-9 Discovery + V0 Contract OWNER APPROVED / CLOSED — V0 core implementation authorized same day.** **ID-9 V0 CORE IMPLEMENTATION COMPLETE — READY FOR OWNER / CHIEF ARCHITECT REVIEW.** New `src/athena/intraday/position_sizing_models.py` (`CapitalPolicy`, `PositionSizing`, `PositionSizingState`/`PositionSizingReasonCode`/`BindingConstraint` enums) and `position_sizing_engine.py` (`PositionSizingV0Engine.evaluate(...)`, pure/deterministic, no repository/provider/network access), consuming `EntryActionability` (state==ACTIONABLE) directly — no new EntryRisk artifact, per the frozen discovery decision. LONG-only (SHORT/NONE refused with `UNVALIDATED_DIRECTION`, NONE additionally treated as a genuine contract error since it is structurally impossible for an ACTIONABLE artifact); three independent constraints (risk-budget, max-position-value, theoretical-deployable-capital), each floored DOWN to whole lots via `instrument.lot_size` (never hardcoded to 1, never rounded up, never forced to a minimum); `binding_constraints` reports every co-binding candidate, never one arbitrary winner. A genuine mathematical finding from implementation: since `max_position_value_pct <= 100%` is enforced, `theoretical_capital_quantity >= max_value_quantity` always — theoretical-capital can only ever co-bind with max-position-value, never uniquely bind alone, reported honestly rather than forcing an impossible test scenario. `CapitalPolicy` is a small, explicit, caller-supplied contract — deliberately NOT `CapitalConfig`/`RiskConfig` themselves (both confirmed dormant/unused by discovery) — `OwnerValidationPipeline` gained an optional `capital_policy` constructor parameter defaulting to `None`, so every real production cycle today honestly reports `CAPITAL_POLICY_UNAVAILABLE` until the Owner explicitly supplies a real policy; no dormant config value is read. `GOAL_BANDS_ONLY`/`RR_INFORMATIONAL_ONLY` preserved — proven by source scan (RR/T1/T2/score/confidence/conviction/RS/RVOL/hit-rate/Kelly never referenced) and behaviorally (an extreme RR fixture produces an identical sizing result). New `position_sizing` `WorkflowStage` (`depends_on=("entry_actionability",)`) added to the canonical per-instrument DAG, consuming the exact same-cycle `EntryActionability` (never a repository "latest" query), gated on out-of-scope Decisions before any composition; no persistence (`PERSISTENCE_NOT_YET_REQUIRED` — schema stays 18), no order/broker/execution stage activated. The dormant P5.2-P5.6 pipeline (`allocation/`, `sizing/`, `orders/`, `brokers/`, `execution/`) is classified `RETAIN_AS_LEGACY_COMPATIBILITY` in full (incompatible allocated-amount-based paradigm vs. ID-9's own risk-budget/multi-constraint model — adapting it would break its own tests for zero benefit since nothing in production calls it) and confirmed, by source scan, never imported by any new ID-9 file — exactly one canonical sizing path exists in production. 42 new tests (34 pure-engine/domain, 8 workflow-integration); full suite **3812 passed, 1 pre-existing unrelated skip, 0 failures**. Zero schema/production changes (`db/athena.db` schema 18, integrity ok, PID 2453 untouched); zero EMR/DarvaX touch; `git diff --check` clean. Classification `ID9_V0_CORE_IMPLEMENTATION_COMPLETE_NO_PRODUCTION_ACTIVATION`. **Owner/Chief Architect source review (2026-09-07): V0 core sizing mathematics and overall architecture ACCEPTED in substance; final closure HELD for three correctness issues, all fixed same milestone, same files (no ID-9.1/ID-9.x created).** (1) **Currentness enforcement**: `EntryActionability.state == ACTIONABLE` is a persisted methodology verdict at evaluation time, not a live-currentness guarantee — same-cycle synchronous production does not waive the frozen ID-7 evidence-age/session-phase contract. `PositionSizingV0Engine.evaluate` now takes a mandatory `currentness: CurrentnessResult` (from the existing, unmodified `is_currently_usable`, never re-implemented) and refuses `NOT_SIZED`/`UPSTREAM_NOT_CURRENT` for anything but `CURRENT`, gated immediately after direction, before capital-policy availability. `position_sizing_stage` composes it via one captured `sizing_clock_instant` (reused for both currentness `now` and `evaluated_at` — proven by a dedicated spy test) and the existing `session.classify_session_phase` helper. (2) **Policy-version identity**: `policy_version` now participates in the artifact's own composite identity via a new `identity_tuple()` method — two sizing assertions over the same upstream checkpoint under different `CapitalPolicy` versions are genuinely different and must never share an identity, while `sizing_methodology_version` remains provably unchanged across policy versions. A refined presence rule (`POLICY_PARTICIPATED_NOT_SIZED_REASON_CODES = {INVALID_RISK_GEOMETRY}`) distinguishes "policy never inspected" (`UPSTREAM_NOT_ACTIONABLE`/`UNVALIDATED_DIRECTION`/`UPSTREAM_NOT_CURRENT`/`CAPITAL_POLICY_UNAVAILABLE`, `policy_version=None`) from "policy inspected but still refused" (`INVALID_RISK_GEOMETRY`, `policy_version` preserved). (3) **Silent lot-size fallback removed**: `position_sizing_stage` previously defaulted `lot_size=1` when canonical `Instrument` metadata was absent from its lookup map; now raises a contract error instead, since that absence is proven architecturally impossible (every instrument reaching the stage was itself sourced from the same `instruments` sequence this scan already resolved) — never sized around with fabricated metadata. `position_sizing_as_of` semantics reviewed and confirmed correct, unchanged (genuinely the right market-time checkpoint, not merely convenient). 19 new tests (16 pure-engine + 3 workflow); 2 pre-existing tests updated (a locked-in literal-count assertion, and a currentness-absence proof re-scoped to `entry_actionability_stage`'s own body since ID-9's own later stage now legitimately reuses currentness concepts) — neither weakened. Full suite **3831 passed, 1 pre-existing unrelated skip, 0 failures** (up from 3812). Zero schema change (stays 18); zero new persistence; dormant P5.2-P5.6 pipeline disposition unchanged (`RETAIN_AS_LEGACY_COMPATIBILITY`); zero EMR/DarvaX/ID-6/ID-7/ID-8-methodology touch; `git diff --check` clean. Classification `ID9_V0_CORE_IMPLEMENTATION_METHODOLOGY_CORRECT_NO_PRODUCTION_ACTIVATION`. Status **ID-9 POSITION SIZING V0 CORE IMPLEMENTATION CORRECTED — READY FOR OWNER / CHIEF ARCHITECT FREEZE DECISION**. Full report: `docs/research/ID-9-POSITION-SIZING-V0-CORE-IMPLEMENTATION.md`. Not marked closed; not activated in production; does not start ID-10. **Owner/Chief Architect decision (2026-09-07): ID-9 V0 core methodology/implementation FROZEN** (classification `ID9_V0_CORE_IMPLEMENTATION_METHODOLOGY_CORRECT_NO_PRODUCTION_ACTIVATION` — `PositionSizing` domain semantics, `PositionSizingV0Engine` mathematics, currentness enforcement, composite identity, policy-participation/provenance rules, LONG-only scope, the three quantity formulas, binding-constraint semantics, Decimal/floor rules, lot-size semantics, zero-quantity behavior, the dormant P5.2-P5.6 disposition, and the persistence decision all frozen unchanged); ID-9 stays OPEN for exactly one remaining operational gap — production always supplies `capital_policy=None` — and **ID-9 V0 CAPITAL POLICY ACTIVATION** authorized same day, forbidding ID-9.1/ID-9.x/ID-10/reopening discovery/touching ID-6/ID-7/ID-8. **ID-9 V0 CAPITAL POLICY ACTIVATION (2026-09-07): READY FOR OWNER VALUES / FINAL CLOSURE.** New `src/athena/intraday/position_sizing_config.py` (own `_Strict` pydantic base + `load_position_sizing_policy_config(config_dir) -> CapitalPolicy | None`, mirroring `athena.explosive_move.live.operational_config`'s own established PATTERN only — own optional file, safe-absence default, unknown keys rejected) is the smallest canonical configuration seam: `config/position_sizing_policy.json` (does not exist in production today), an explicit 4-field shape (`total_deployable_capital`/`risk_budget_per_trade_pct`/`max_position_value_pct`/`policy_version`, all three numeric fields mandatory JSON strings for exact Decimal parsing — a bare JSON number is rejected outright), wired into the ONE canonical production construction seam both `_cmd_cycle` and `_cmd_serve`'s scheduled worker already share (`cli.py`'s `_owner_validation_pipeline` helper) — `OwnerValidationPipeline.__init__` itself is completely unchanged. Absent file → `None` → `CAPITAL_POLICY_UNAVAILABLE` unchanged (proven); present-and-valid → exactly one immutable `CapitalPolicy` constructed and reused for every instrument in a scan (proven by object-identity spy across two real instruments); present-and-invalid → `ConfigError`, never a silent fallback. `config/capital.json`/`config/risk.json` (confirmed dormant by the ID-9 discovery report) are proven, by source-scan and behavioral test, never read by this loader. Produced the required non-prescriptive Owner Policy Impact Table (normalized ₹10,00,000 capital, entry=₹100, LOW/MEDIUM/WIDE per-share-risk ₹1/₹2/₹4 × the specified 3×3 `risk_budget_per_trade_pct`/`max_position_value_pct` matrix, 27 rows, generated by calling the real frozen engine, never hand-derived) via `docs/research/ID-9-POSITION-SIZING-V0-CORE-IMPLEMENTATION.md`'s new §36-§49 — reconfirming, never re-deriving, the already-proven property that `THEORETICAL_CAPITAL` never uniquely binds (always ≥ `MAX_POSITION_VALUE`'s quantity). Policy-version discipline (§40) is documented as an operational invariant ("any effective policy value change must pair with a new `policy_version`"), not enforced via a new registry/persistence subsystem, per the authorization's own explicit prohibition. **37 new tests** (31 pure config-loader unit tests in `tests/market_intel/test_position_sizing_config.py`, 6 workflow-seam integration tests in `tests/ops/test_owner_validation.py`) — none of the 61 pre-existing ID-9 tests weakened. Full suite **3868 passed, 1 pre-existing unrelated skip, 0 failures** (up from 3831). Zero schema change (stays 18); zero new persistence; zero order/broker/execution/provider/network call from policy loading (proven); `db/athena.db` unchanged (`integrity_check: ok`); PID 2453 untouched; EMR/DarvaX untouched; `git diff --check` clean (diff scoped to `cli.py`, `tests/ops/test_owner_validation.py`, and two new files). Production `capital_policy` remains intentionally absent — `config/position_sizing_policy.json` was NOT created and NO numeric value was invented. Exact four values still required from the Owner: (A) `total_deployable_capital`, (B) `risk_budget_per_trade_pct`, (C) `max_position_value_pct`, (D) `policy_version` — once supplied, activation is a one-file, zero-code, zero-restart configuration change (§49). Status **ID-9 V0 CAPITAL POLICY ACTIVATION READY FOR OWNER VALUES / FINAL CLOSURE**. Not marked closed; does not start ID-10 |
