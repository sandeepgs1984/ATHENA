...
def test_dashboard_modals_are_inert_outside_tab_flow(client: TestClient) -> None:
    ...
    ) == 2  # loadDecisionsWorkspace() and refreshDecisionCacheForSymbols()
    ...
    assert "dashboard.css?v=9.231.0" in html
    assert "dashboard.js?v=9.231.0" in html
...
def test_my_portfolio_dashboard_tab_contract(client: TestClient) -> None:
    ...
    assert 'id="my-portfolio-export-toggle"' in html
    ...
    assert html.find('class="my-portfolio-command-dashboard"') < html.find(
        'class="my-portfolio-risk-panel collapsed"'
    )
    assert html.find('class="my-portfolio-risk-panel collapsed"') < html.find(
        'class="my-portfolio-heatmap-panel collapsed"'
    )
    assert html.find('class="my-portfolio-heatmap-panel collapsed"') < html.find(
    ...
    assert '@import url("css/05b-my-portfolio.css?v=9.231.0");' in css_manifest
    ...
    assert ".my-portfolio-detail-row-wide" in my_portfolio_css
    assert ".my-portfolio-change-delta-stack" in my_portfolio_css
    assert "grid-template-columns: repeat(2, minmax(0, 1fr))" in my_portfolio_css
    ...
    assert "Unavailable: proximity to a trigger is not currently assessed." in html
    ...
    assert "Latest snapshot matches the previous one" in js
    assert "Earlier snapshot history. The latest pair is in Since last sync." in js
    assert "myPortfolioRiskLabel(raw)" in js
    ...
    assert "riskExpanded: false" in js
    ...
    open_decision_fn_body = js[open_decision_fn_start:open_decision_fn_start + 4500]
