...
| ID-6 | Discovery / scope and architecture freeze — determine what ID-6 should be from the live architecture and the completed ID-0→ID-5 evidence, especially the owner-approved ID-5B `CASE_B_CONTENT_CHANGES` result | 🔄 Ready for owner architecture review 2026-09-02 — design report recommends GO WITH CONDITIONS: proceed toward Entry Qualification, but start with ID-6A domain/state/reliability contracts and an explicit live-M5 provisional-evidence policy before any production qualification engine, persistence, UI, thresholds, Entry/TradePlan, ID-7, EM-6, EMR, DarvaX, or order behavior. Evidence: `docs/research/ID-6-SCOPE-ARCHITECTURE-DESIGN.md` |
