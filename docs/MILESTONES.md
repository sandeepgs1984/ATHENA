...
| EM-6 | Add the EMR research UI only after scanner approval | 🔄 **DISCOVERY OWNER-APPROVED / CLOSED; SCOPE RATIFIED 2026-09-03** — EM-6 formally ratified as a read-only, permanently "Experimental" EMR research presentation milestone (not modeling — EM-4B/4D/4E already closed fitting/calibration/sealed-holdout), executed in two gated slices: EM-6A (read-only presentation data contract) and EM-6B (API mount + dashboard, not yet authorized). Full discovery contract: `docs/research/EM-6-DISCOVERY-AND-MODELING-CONTRACT.md`. EM-6A is implemented and awaiting owner review — see the EM-6A row below. EM-6 overall remains open pending EM-6B authorization |
| EM-6A | Read-only EMR presentation data/query contract — no HTTP, no dashboard, no scanner scheduling | ✅ **IMPLEMENTATION COMPLETE — READY FOR OWNER REVIEW 2026-09-03** — new `src/athena/explosive_move/live/presentation.py`: `latest_scan_snapshot()`, `top_candidates()`, `top_touch_10_candidates()`, `coverage_summary()`, `describe_scan_freshness()` (pure, `as_of`-explicit, no `datetime.now()`), `build_experimental_snapshot()`. Audited the EM-5 contract's described `top_candidates`/`top_touch_10_candidates` seam and found neither was ever actually implemented (design-doc prose only) — this is the first real implementation, built directly against the frozen `emr_scan_runs`/`emr_candidates` schema. Uses its own genuinely read-only SQLite connection (`mode=ro`+`PRAGMA query_only=ON`, mirroring the ID-track's own shadow-audit pattern) rather than the read-write `EmrRepository`, giving a structural (not conventional) write-impossibility guarantee — proven by a direct `sqlite3.OperationalError` test. Every candidate returned is scoped to one explicit `run_id`, always sourced from this module's own latest-scan lookup — cross-scan mixing is structurally unreachable, not merely avoided (2 mutation-verified: COMPLETE-status filter, run_id coherence filter). Empty states (no db file, no completed scan, zero ranked candidates) are well-defined return values, never exceptions or fabricated rows. 24 new focused tests covering all 15 owner-required categories, all passing; combined `tests/explosive_move/` suite 421 passed (unmodified pre-existing isolation/no-model-learning architecture tests automatically cover the new file too). Full repository suite 3,214 passed, 1 pre-existing skip. Ruff clean, `git diff --check` clean. Real-data acceptance check against production `db/athena.db`-adjacent `db/emr.db`: **REAL_DATA_ACCEPTANCE_NOT_AVAILABLE** — the file does not exist in production (scanner never run there, no scheduler trigger exists, none added). No provider/network calls, no canonical ATHENA/DarvaX import (grep-confirmed and architecture-test-confirmed). Full contract: `docs/design/EM-6A-READ-ONLY-PRESENTATION-DATA-CONTRACT.md`. EM-6B not started |
