...
| `docs/research/PS-P0-PORTFOLIO-SYNC-DISCOVERY-REPORT.md` + `docs/research/PS-P1-PORTFOLIO-CONTRACT-DESIGN.md` + `docs/research/PS-P2-PORTFOLIO-IMPORT-RECONCILIATION.md` + `docs/research/PS-P3-MY-PORTFOLIO-DASHBOARD-UX.md` | Portfolio Sync / My Portfolio discovery, frozen contract design, import/reconciliation implementation, and dashboard upload UX: isolated source of truth, CSV/XLSX preview, canonical symbol resolution, atomic confirmation, stale-preview recovery, factual current-holdings display, freshness/provenance, nullable methodology fields, and 20-column Portfolio Snapshot DTO | Starting or reviewing any My Portfolio / Portfolio Sync milestone; confirm current PS status in `docs/MILESTONES.md` first |
...
| Intraday Intelligence (ID-track, foundation/evidence only — no EntryQualification, no Decision gate) | `session/` (ID-1: `SessionContext`, completed-candle rule, `session_day_start`/`session_open_close_ts`/`canonical_slot_candles` — the shared completed + canonical-slot authorities every consumer below reuses) and `intraday/` — `IntradaySignalSet` (ID-2, formalizes existing VWAP/5m-15m-confluence evidence), `OpeningRangeEvidence` (ID-3/ID-3.1: OR15/OR30, canonical-slot-integrity-corrected), `RelativeStrengthContext` (ID-4/ID-4.1, Owner-approved: stock-vs-sector/market point-in-time comparative performance, not RSI — comparative dimensions data-blocked pending ID-5's index M5 remediation, check `docs/MILESTONES.md`), `GapContext` (ID-5C, Owner-approved/CLOSED: previous-trading-session-close → current-session-open price transition, D1-only, deliberately independent of ID-5B's now-closed live M5 semantics question), `RelativeVolumeContext` (ID-5D methodology Owner-accepted; ID-5D.1 owner-approved/CLOSED: corrected current-window contiguity + retrieval-policy correctness fixes on top of ID-5D's accepted cumulative same-time-of-day relative volume, no hardcoded baseline-length cap, canonical-M5-only so deliberately independent of ID-5B too). ID-5B and ID-5 are owner-approved/CLOSED as of 2026-09-01 with final `CASE_B_CONTENT_CHANGES`; ID-6 remains not started and requires separate owner-authorized design — check `docs/MILESTONES.md`. Wired into `OwnerValidationPipeline._scan_eligible`'s per-instrument `WorkflowStage` graph (`session` also produces `gap_context`; `intraday_analytics`/`relative_strength`/`relative_volume`), never scoring/confidence/risk/decision |
...
| Portfolio & planning (no execution) | `portfolio/` — existing portfolio engine plus My Portfolio contracts (`my_portfolio_contracts.py`) and PS-P2 generic import parsing/resolution (`imports.py`) for CSV/XLSX current-holdings preview, canonical symbol resolution, reconciliation, server-owned snapshot math, sync state, freshness, and provenance; isolated `portfolio_*` SQLite tables live in `data/store/schema.py`, API endpoints live under `/api/v1/my-portfolio`, PS-P3 dashboard UX lives in `api/static/js/08b-my-portfolio.js` and `api/static/css/05b-my-portfolio.css`, and `owner_positions` remains the legacy/manual-fill ledger. Also `allocation/`, `sizing/`, `orders/` (planning only), `brokers/` (abstraction only), `execution/` |
