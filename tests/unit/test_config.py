...
def test_per_trade_risk_exceeding_daily_loss_is_rejected(config_dir):
    ...
    with pytest.raises(ConfigError, match="per_trade_risk_pct.*max_daily_loss_pct"):
...
def test_position_cap_exceeding_sector_cap_is_rejected(config_dir):
    ...
    with pytest.raises(ConfigError, match="max_capital_per_position_pct"):
        load_config(config_dir)
...
def test_unknown_profile_lists_available(config_dir):
    with pytest.raises(ConfigError, match="not found.*Available.*intraday-momentum"):
...
def test_unversioned_profile_indicator_is_rejected(config_dir):
    ...
    with pytest.raises(ConfigError, match="unversioned indicators.*supertrend"):
        load_config(config_dir)
...
def test_unknown_key_is_rejected_as_typo(config_dir):
    ...
    with pytest.raises(ConfigError, match="max_dialy_loss_pct"):
        load_config(config_dir)


def test_missing_file_is_a_readable_error(config_dir):
    (config_dir / "capital.json").unlink()
    with pytest.raises(ConfigError, match="Missing configuration file.*capital.json"):
        load_config(config_dir)
