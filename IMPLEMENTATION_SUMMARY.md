...
| Files created | `src/athena/darvax/api/static/tab.js`, `tests/darvax/test_dx4b_tab.py`, `tests/darvax/_tab_harness.js` |
| Files modified | `src/athena/api/static/index.html` (one `<script>` tag), `src/athena/api/app.py` (seam call now passes `config_dir`/`repo_root` — see the defect below), `src/athena/darvax/api/static/darvax.js` (`embedded=1` handling), `tests/darvax/test_dx4_surface.py` (scope note for the one permitted reference), `docs/adr/ADR-010-darvax-satellite-module.md` (Amendment 1 → Accepted), `docs/MILESTONES.md`, `IMPLEMENTATION_SUMMARY.md` |
