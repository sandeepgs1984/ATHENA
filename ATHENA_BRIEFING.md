...
| Explosive Move research (isolated, ADR-012) | `explosive_move/` — EM-1a immutable event/readiness contracts, EM-1r2 read-only corporate-action coverage/cohort evidence, and EM-1r3 provider-free exact-slot intraday reconstruction/manifests; no dataset labels, features, model, scanner, UI, canonical score input, or production recommendation |
