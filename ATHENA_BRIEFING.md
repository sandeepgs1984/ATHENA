...
| `docs/research/PS-P0-PORTFOLIO-SYNC-DISCOVERY-REPORT.md` + `docs/research/PS-P1-PORTFOLIO-CONTRACT-DESIGN.md` | Portfolio Sync / My Portfolio discovery and frozen contract design: isolated source of truth, import/reconciliation model, sync contract, freshness/provenance, nullable methodology fields, and 20-column Portfolio Snapshot DTO | Starting or reviewing any My Portfolio / Portfolio Sync milestone; confirm current PS status in `docs/MILESTONES.md` first |
...
| Portfolio & planning (no execution) | `portfolio/` — existing portfolio engine plus PS-P1 My Portfolio contracts (`my_portfolio_contracts.py`) for imported current holdings, reconciliation, server-owned snapshot math, sync state, freshness, and provenance; isolated `portfolio_*` SQLite tables live in `data/store/schema.py` and `owner_positions` remains the legacy/manual-fill ledger. Also `allocation/`, `sizing/`, `orders/` (planning only), `brokers/` (abstraction only), `execution/` |
