...
| `docs/research/PS-P0-PORTFOLIO-SYNC-DISCOVERY-REPORT.md` + `docs/research/PS-P1-PORTFOLIO-CONTRACT-DESIGN.md` + `docs/research/PS-P2-PORTFOLIO-IMPORT-RECONCILIATION.md` + `docs/research/PS-P3-MY-PORTFOLIO-DASHBOARD-UX.md` + `docs/research/PS-P4-PORTFOLIO-SYNC-ORCHESTRATION.md` + `docs/research/PS-P5A-PORTFOLIO-INTERPRETATION-METHODOLOGY.md` + `docs/research/PS-P5B-PORTFOLIO-INTERPRETATION-IMPLEMENTATION.md` + `docs/research/PS-P6C-MY-PORTFOLIO-V1-END-TO-END-VALIDATION.md` + `docs/research/PS-P7A-PORTFOLIO-INTELLIGENCE-V2-DISCOVERY.md` | Portfolio Sync / My Portfolio V1 discovery through final freeze, plus Portfolio Intelligence V2 discovery: isolated source of truth, CSV/XLSX preview, canonical symbol resolution, atomic confirmation, stale-preview recovery, immutable analysis snapshots, factual current-holdings display, server-owned valuation math, expected-session freshness/provenance, coherent Decision/TradePlan evidence, frozen V1 portfolio interpretation, holdings-digest currentness, final V1 validation, and V2 evidence/methodology inventory for Conviction, Trend / Setup, Support 1, Target 2/3, REDUCE, ROTATE, history, coherency, and versioning | Starting or reviewing any My Portfolio / Portfolio Sync / Portfolio Intelligence milestone; confirm current PS status in `docs/MILESTONES.md` first |
...
| Portfolio & planning (no execution) | `portfolio/` — existing portfolio engine plus frozen My Portfolio V1 contracts (`my_portfolio_contracts.py`), PS-P2 generic import parsing/resolution (`imports.py`), PS-P4 sync orchestration (`sync.py`), and PS-P5B interpretation (`interpretation.py`) for CSV/XLSX current-holdings preview, canonical symbol resolution, reconciliation, server-owned snapshot math, background sync state, immutable analysis snapshots, freshness, provenance, holdings-digest currentness (`CURRENT` / `STALE_HOLDINGS_CHANGED` / `UNKNOWN`), Status, Key Trigger, Major Support / Exit, Target 1, Next Action, EntryQualification-gated ADD, inclusive stop-driven EXIT, PARTIAL/FAILED handling, stale snapshot preservation, and compact explanation UX. My Portfolio V1 is COMPLETE AND FROZEN as of 2026-09-03 after PS-P6C; PS-P7A is discovery-only for Portfolio Intelligence V2 and implements no V2 field. Isolated `portfolio_*` SQLite tables live in `data/store/schema.py`, API endpoints live under `/api/v1/my-portfolio`, dashboard UX lives in `api/static/js/08b-my-portfolio.js` and `api/static/css/05b-my-portfolio.css`, and `owner_positions` remains the legacy/manual-fill ledger. Also `allocation/`, `sizing/`, `orders/` (planning only), `brokers/` (abstraction only), `execution/` |
