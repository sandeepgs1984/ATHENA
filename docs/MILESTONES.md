...
| EM-6 | Add the EMR research UI only after scanner approval | 🔄 **DISCOVERY AUTHORIZED / DISCOVERY COMPLETE — READY FOR OWNER CONTRACT REVIEW 2026-09-03** — source-grounded discovery (no implementation) found that EM-4B/4D/4E already closed model fitting/calibration/sealed-holdout evaluation, so EM-6 is not a modeling milestone; the only existing EM-6 spec anywhere is roadmap-intent (`docs/design/ATHENA-EXPLOSIVE-MOVE-RADAR-ROADMAP.md` §9): a read-only, permanently "Experimental"-labelled EMR research UI, never a modeling step. Recommended shape: **G. OTHER** (read-only presentation), proposed as two gated sub-milestones (EM-6A read-only query layer, EM-6B dashboard) — neither implemented. Full leakage/determinism/model-status audit found zero gaps requiring a fix; FINAL_TEST confirmed already opened once under its sealed one-shot policy (not untouched) and must never be reread. ID-track and DarvaX isolation reconfirmed, zero coupling. Full contract: `docs/research/EM-6-DISCOVERY-AND-MODELING-CONTRACT.md`. Owner decision required before any implementation |
