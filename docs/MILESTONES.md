...
| ID-6 | Discovery / scope and architecture freeze — determine what ID-6 should be from the live architecture and the completed ID-0→ID-5 evidence, especially the owner-approved ID-5B `CASE_B_CONTENT_CHANGES` result | ✅ **DISCOVERY ARCHITECTURE OWNER APPROVED WITH CONDITION 2026-09-02** — corrected design direction and architecture accepted, conditional on completing ID-6A0 before any ID-6A implementation. State and reliability remain orthogonal, current-session M5 remains provisional, and no irreversible ID-6 state may be caused directly or indirectly solely by live-M5-provisional evidence. No production qualification engine, persistence, UI, thresholds, Entry/TradePlan, ID-7, EM-6, EMR, DarvaX, or order behavior authorized. Evidence: `docs/research/ID-6-SCOPE-ARCHITECTURE-DESIGN.md` |
| ID-6A0 | Entry Qualification Architecture ADR — freeze the new persisted decision-relevant concept, advisory-only boundary, daily Decision vs intraday actionability separation, state/reliability orthogonality, live-M5 provisionality invariant, workflow-stage boundary, persistence/auditability principles, and owner-gated ID-6 implementation sequence | 🔄 **READY FOR OWNER ADR REVIEW 2026-09-02** — standalone ADR proposed as `docs/adr/ADR-013-entry-qualification-architecture.md`. ID-6A0 is documentation/governance only: no ID-6A Python types, schema, repositories, engines, workflow wiring, thresholds, UI, provider calls, DB writes, production behavior, EMR, DarvaX, or order behavior implemented |
