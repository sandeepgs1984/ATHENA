...
| EM-7 | 🔄 EM-7A0 (ADR-014) ACCEPTED 2026-09-03; **EM-7A OWNER APPROVED / CLOSED 2026-09-03 (via EM-7A.1 + EM-7A.2); EM-7B OWNER APPROVED / CLOSED 2026-09-04 (via EM-7B.1); EM-7C CONTROLLED PRODUCTION ACTIVATION AUTHORIZED, CANARY EVIDENCE ACCEPTED, CLOSURE HELD FOR EM-7C.1; EM-7C.1 PRODUCTION MOUNT FAIL-CLOSED ISOLATION COMPLETE — EM-7C READY FOR OWNER / CHIEF ARCHITECT CLOSURE REVIEW; EM-7D NOT AUTHORIZED** — isolated shadow validation, OFF-vs-shadow comparison (ADR-012 §10). Discovery found the actual gap after EM-6 is entirely operational, plus 3 real scanner-correctness gaps blocking unattended operation. Owner ratified 5 architecture decisions and accepted `docs/adr/ADR-014-emr-live-shadow-operation.md`. EM-7A's mandatory pre-implementation persistence audit found `save_candidates`/`save_transitions`/the terminal `COMPLETE` write were three separate, non-transactionally-linked SQLite transactions — contradicting ADR-014 §15's original atomicity assumption; a failure between any two could leave a durable partial result. Reported per explicit owner instruction rather than silently resolved; EM-7A that day independently completed and tested the EMR-owned `flock` concurrency lock (`explosive_move/live/scan_lock.py`, no canonical `ops`/`scheduling` dependency), `regime_lookup` hardened to a required parameter (no silent all-UNKNOWN fallback), and isolation-test coverage extended (`scoring`/`confidence`/`intraday`/`darvax`/`ops`/`scheduling` + a live-runtime `SqliteRepository` boundary test) — then stopped on lifecycle/retry/idempotency/transactional-consistency, recommending Option 1 (one shared transaction). **EM-7A.1 (same day) is the owner's resolution: Option 1 selected, `PARTIAL` lifecycle state rejected.** `EmrRepository.commit_scan_result` now wraps candidate + transition + terminal-`COMPLETE` persistence in one atomic SQLite transaction (verifies `RUNNING` first, delete-then-insert replace-for-run); `run_scan_cycle` calls it once. `mark_scan_failed` writes an explicit `FAILED` status (separate transaction, bounded diagnostics) for every run-level exception after `RUNNING`, exception-chaining a secondary marking failure without masking the original. Three same-`run_id` idempotency cases handled before any provider call (`COMPLETE`→reconstruct with zero recompute, `RUNNING`→reject via `EmrScanAlreadyRunningError`, `FAILED`→retry same `run_id`); a `UNIQUE(run_id, instrument_id, family, threshold_percent)` index (schema v2) is defense-in-depth only. `run_scan_cycle_with_lock` is the one hardened entrypoint for a future EM-7B worker. Atomicity proven by direct mutation/negative test (temporarily reverting to three separate transactions correctly failed the dedicated rollback/one-transaction-boundary tests; reverted). ADR-014 §15/§16 corrected to describe the true before/after architecture, preserving the original (superseded) claim and the EM-7A finding that disproved it. Checkpoint-constant consolidation deliberately deferred (the "duplicate" is an intentional independent-verification pin, per its own docstring). **EM-7A.1 source-reviewed and technically accepted 2026-09-03; EM-7A closure held for one narrow mismatch: `run_scan_cycle` still persisted a fourth status, `SKIPPED_SESSION_TYPE`, on a non-scannable session (RUNNING written then immediately overwritten), contradicting ADR-014's accepted two-terminal-outcome model. EM-7A.2 (same day) is the owner's resolution** — session-scannability now checked as a true preflight, after the existing-run dispatch (COMPLETE/RUNNING/FAILED-or-legacy-SKIPPED unchanged) but before any RUNNING write, provider call, or computation; a non-scannable session returns an in-memory-only `SKIPPED_SESSION_TYPE` outcome, persisting nothing. New executions can persist only RUNNING/COMPLETE/FAILED; a pre-EM-7A.2 database's legacy SKIPPED rows remain readable (same-run_id lookup only, never written again). COMPLETE-authoritative, RUNNING-rejection, and FAILED-not-mutated-by-a-skip-retry semantics all preserved, proven by 6 new tests plus a required mutation/negative proof (reverted). Full `tests/explosive_move/` + `tests/api/v1/test_emr_router.py`: 473 passed (was 467). Full repository suite: 3,285 passed, 1 skipped (pre-existing, unrelated). **Owner/Chief Architect closed EM-7A.2 and EM-7A themselves 2026-09-03 and authorized EM-7B same day.** **EM-7B (2026-09-03)** implements the isolated EMR scheduling/invocation layer (`docs/research/EM-7B-ISOLATED-SCHEDULING-INVOCATION.md`, `src/athena/explosive_move/live/worker.py`): a pure, fully injectable `run_once` tick (checkpoint-due derivation, latest-due-only catch-up after an audited source search found no methodological requirement to back-fill missed checkpoints, restart recoverability from persisted `emr_scan_runs` state, FAILED never auto-retried) plus a thin `EmrWorker` daemon-thread wrapper mirroring `CycleWorker`'s shape without importing it; a new `_MatureHistoryMarketDataPort` wrapper resolves the frozen mature-history universe policy without touching `run_scan_cycle`'s own contract; gated on a new `config/emr/operational.json` (`enabled: false` shipped default). `compute_run_id` extracted from `scanner.py` as a small, behavior-preserving public function. 3 required mutation/negative proofs (mature-filter bypass, FAILED-retry-gate removal, forbidden import) all caught correctly, reverted. Isolation test extended to approve `worker.py` as a second read-only `SqliteRepository` importer. Full `tests/explosive_move/` + `test_emr_router.py`: 502 passed (was 473). Full repository suite: 3,314 passed, 1 skipped (pre-existing, unrelated). **Owner/Chief Architect source review provisionally accepted EM-7B but held closure for one narrow configuration-authority issue** (2026-09-04): `max_checkpoint_price_delay_seconds` was operator-tunable in `config/emr/operational.json` even though its value (`300.0`) exactly equals `checkpoint_reference_price.MAX_CHECKPOINT_OBSERVATION_DELAY_SECONDS`, an owner-approved bound whose own docstring says "EM-5 must not dynamically retune this." **EM-7B.1 (same day) is the owner's resolution:** the field is removed entirely; `worker.py` now imports and uses the frozen constant directly. `max_staleness_minutes` stays configurable (genuinely operational per `eligibility.py`'s own docstring), its default now regression-tested against the real accepted canary's own signature. `base_universe`/`model_version` remain selectors but `load_emr_operational_config` now rejects any value that does not resolve to an already-frozen source (`athena_core` per ADR-014 §11; a `model_version` whose promoted manifest exists and agrees). Mutation/negative proof: disabling the new validation correctly failed exactly the 3 tests proving it; reverted. No ADR-014 change needed. Full `tests/explosive_move/` + `test_emr_router.py`: 514 passed (was 502). Full repository suite: 3,326 passed, 1 skipped (pre-existing, unrelated). **Owner/Chief Architect closed EM-7B.1 and EM-7B themselves 2026-09-04 and authorized EM-7C same day — the FIRST controlled production activation of the isolated EMR live-shadow path.** **EM-7C (2026-09-04)** added `_mount_emr_worker` (`src/athena/cli.py`), called from `_cmd_serve` alongside (never through) the existing canonical `CycleWorker` — independent lock, independent repository, isolated failure handling, zero side effects when disabled (mutation-proven). After a full read-only pre-activation audit, the real production service (PID 93626, healthy, running since 2026-09-03) was gracefully restarted via the official `./athena-serve` wrapper with `config/emr/operational.json` set `enabled: true`. `db/emr.db` was created intentionally for the first time (schema v2). **The genuine scheduled canary occurred naturally at the 09:20 IST checkpoint** (`EmrWorker → run_once → run_scan_cycle_with_lock → run_scan_cycle`, no manual invocation) and completed `COMPLETE`, atomic, with an honest zero-eligible result: 0 of 518 mature-history instruments had any canonical M5 candle data yet at that exact instant, root-caused to canonical ingestion's own scheduling (`config/scheduling.json`) not yet having landed the day's first bar for the full universe — a genuine cross-system timing characteristic, not an EM-7A/B/C defect. Real Kite `/quote` traffic occurred correctly (136 requests, ~301.5s, within the frozen `MAX_CHECKPOINT_OBSERVATION_DELAY_SECONDS` bound). Canonical DB/runtime, ID-7P0, and DarvaX all verified isolated and unaffected; EM-6's `latest_scan_snapshot` correctly represents the COMPLETE scan even with zero candidates. 6 new focused tests plus a required mutation/negative proof, reverted. Full repository suite: 3,332 passed, 1 skipped (pre-existing, unrelated), 0 failed. `enabled=true` left in place for natural EM-7D evidence accumulation (not yet started). **Owner/Chief Architect source review accepted the canary as genuine but held EM-7C closure for one narrow isolation defect: `_mount_emr_worker` loaded/validated `EmrOperationalConfig` outside its own protective `try` block, so an EMR-specific config failure could have crashed canonical `_cmd_serve`. EM-7C.1 (same day) is the owner's resolution** — the whole EMR mount sequence now lives inside one `try` block (EMR fails closed for itself, open for canonical ATHENA); invalid config surfaces as one bounded warning, never a silent "disabled" reinterpretation. 7 new tests plus a required mutation/negative proof (config load moved back outside the boundary; reverted, confirmed identical). Incidentally fixed a latent `test_em7b_worker.py` lock-path collision with the now-real production `EmrScanLock` (`LOCK_BUSY` observed directly once EM-7C activated). No new canary triggered; existing 09:20 evidence remains accepted; no production restart performed solely for this fix. Full `tests/explosive_move/` + `test_emr_router.py` + `test_em7c_service_mount.py`: **527 passed**. Full repository suite: **3,339 passed** (was 3,332), 1 skipped, 0 failed. Ruff clean; `git diff --check` clean. Full detail: `docs/research/EM-7-DISCOVERY.md`, ADR-014, `docs/research/EM-7A-SCANNER-CORRECTNESS-HARDENING.md`, `docs/research/EM-7B-ISOLATED-SCHEDULING-INVOCATION.md`, `docs/research/EM-7C-PRODUCTION-ACTIVATION-CANARY.md`. **Owner/Chief Architect decision (2026-09-05): EM-7C OWNER APPROVED / CLOSED, EM-7C.1 OWNER APPROVED / CLOSED -- natural EMR production shadow accumulation active.** EM-7D0 (read-only evidence-readiness audit -- no methodology/threshold/checkpoint/universe/provider change, no artificial/backdated scans, no restart) authorized and completed same day: audited the complete natural 2026-09-04 session (all 9 frozen checkpoints present) read-only against `db/emr.db`. 9/9 runs COMPLETE, 0 duplicate run identities, 0 orphan candidates/transitions, 0 duplicate candidate/transition identities -- sound. **Current-session M5 availability recovered by 09:30** (the very next checkpoint after the already-accepted 09:20 canary) and held through 11:00, then experienced one further, genuinely distinct 0-eligible episode at **12:00** -- unlike 09:20 (near-zero evidence-generation/inference duration, zero candidate rows), the 12:00 episode shows a fully-scored 517-instrument population (real logits/regime features, non-NULL checkpoint prices) uniformly flagged `state_reason='STALE_DATA'` by the M5-staleness gate specifically -- correlated with a real canonical REFRESH cycle that FAILED at 11:35:48 and did not COMPLETE again until 11:59:46 (a ~24+ minute gap landing across the checkpoint), consistent with ID-7P0's own already-measured REFRESH latency -- a cross-system timing characteristic, not an EMR defect. Regime lookup confirmed exercised with real, checkpoint-specific data (sampled logit contributions show genuine non-trivial regime one-hot terms). Full real state-machine population observed (all 8 states); genuine TOUCH-10 TARGET_REACHED evidence (6 observations). No profitability/outcome analysis performed. **Classification: B -- `OPERATIONALLY_SOUND_BUT_NOT_YET_STATISTICALLY_READY`** (integrity sound, every subsystem exercising real logic on real data, but only one calendar session exists -- no invented minimum session count). Natural accumulation left unchanged and active; ID-7F2 confirmed untouched (still PRE-ACTIVATION PREPARATION COMPLETE / DEFERRED / OPEN); DarvaX untouched. Full report: `docs/research/EM-7D0-EVIDENCE-READINESS-FIRST-PRODUCTION-SHADOW-AUDIT.md`. Not marked Owner-approved. **EM-7D statistical methodology validation remains NOT STARTED, NOT AUTHORIZED** |
